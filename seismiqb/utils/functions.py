...
@njit
def filter_simplices(simplices, points, matrix, threshold=5.0):
...
def make_bezier_figure(
    ...
    p = np.arctan(sharpness) / np.pi + 0.5
    ...
    angles = (
        p * angles
        + (1 - p) * rolled_angles
        + np.pi * (np.abs(rolled_angles - angles) > np.pi)
    )
    ...
    middle_control_points_1 = (
        np.transpose(radii * [np.cos(angles[:-1]), np.sin(angles[:-1])])
        + key_points[:-1]
    )
    middle_control_points_2 = (
        np.transpose(radii * [np.cos(angles[1:] + np.pi), np.sin(angles[1:] + np.pi)])
        + key_points[1:]
    )
    curve_main_points_arr = np.hstack(
        [
            key_points[:-1],
            middle_control_points_1,
            middle_control_points_2,
...
def trinagular_kernel_1d(length, alpha=0.1):
...
def triangular_weights_function_nd(array, alpha=0.1):
    ...
    for i, axis_len in enumerate(array.shape):
        if axis_len != 1:
            ...
            result = result * trinagular_kernel_1d(axis_len, alpha).reshape(
                multiplier_shape
            )
    return result
...
def insert_points_into_mask(mask, points, mask_bbox, width, axis, alpha=1):
    ...
    for i in prange(len(points)):
        point = points[i]
        if (
            (point[0] >= mask_bbox[0][0] - left_margin[0])
            and (point[1] >= mask_bbox[1][0] - left_margin[1])
            and (point[2] >= mask_bbox[2][0] - left_margin[2])
            and (point[0] < mask_bbox[0][1] + right_margin[0] - 1)
            and (point[1] < mask_bbox[1][1] + right_margin[1] - 1)
            and (point[2] < mask_bbox[2][1] + right_margin[2] - 1)
        ):
