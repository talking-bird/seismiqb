...
class Accumulator3D:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dtype = dtype
        self.transform = (
            getattr(self, transform) if isinstance(transform, str) else transform
        )
    ...
    def create_placeholder(self, name=None, dtype=None, fill_value=None):
        ...
        if self.type in ["hdf5", "qhdf5"]:
            placeholder = self.file.create_dataset(
                name,
                shape=self.shape,
                dtype=dtype,
            ...
            kwargs = {"chunks": (1, *self.shape[1:]), **self.dataset_kwargs}
            placeholder = self.file.create_dataset(
                name, shape=self.shape, dtype=dtype, fill_value=fill_value, **kwargs
            )
    ...
    def __setstate__(self, state):
        ...
        if self.type in ["hdf5", "qhdf5", "zarr"]:
            ...
            for name in self.placeholders:
                ...
                placeholder = np.frombuffer(
                    blosc.decompress(compressed, True), dtype=dtype
                ).reshape(shape)
                setattr(self, name, placeholder)
    ...
    def update(self, crop, location):
        ...
        for s, slc in zip(crop.shape, location):
            ...
            if s < slc.stop - slc.start:
                raise ValueError(
                    f"Inconsistent crop_shape {crop.shape} and location {location}"
                )
        ...
        for xmin, slc, xmax in zip(self.origin, location, self.shape):
            ...
            loc_crop.append(
                slice(
                    max(0, xmin - slc.start),
                    min(xmax + xmin - slc.start, slc.stop - slc.start),
                )
            )
    ...
    def export_to_hdf5(
        ...
        if self.type != "numpy" or self.orientation != 0:
            raise NotImplementedError(
                "`export_to_hdf5` works only with `numpy` accumulators with `orientation=0`!"
            )

        # Parse parameters
        from ..geometry.conversion_mixin import (
            ConversionMixin,
        ...
        with h5py.File(path, mode="w-") as file:
            with Notifier(
                pbar, total=sum(data.shape[axis] for axis in projections)
            ) as progress_bar:
                for axis in projections:
                    projection_name = ConversionMixin.PROJECTION_NAMES[axis]
                    projection_transposition = (
    ...
    def from_aggregation(
        ...
        **kwargs,
        ...
        aggregation_to_class = {
            alias: class_
            for class_, lst in class_to_aggregation.items()
            for alias in lst
        }

        return aggregation_to_class[aggregation](
            shape=shape,
            origin=origin,
            dtype=dtype,
...
class MaxAccumulator3D(Accumulator3D):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        min_value = (
            np.finfo(dtype).min if "float" in dtype.__name__ else np.iinfo(dtype).min
        )
        self.fill_value = fill_value if fill_value is not None else min_value
        self.create_placeholder(
            name="data", dtype=self.dtype, fill_value=self.fill_value
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if dtype in [np.int8, np.uint8]:
            raise NotImplementedError(
                "`mean` accumulation is unavailable for one-byte dtypes."
            )
        super().__init__(
            shape=shape,
            origin=origin,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if dtype not in [np.float32, np.float64]:
            raise ValueError(
                "Dtype should be float32 or float64 for `std` accumulator!"
            )
        super().__init__(
            shape=shape,
            origin=origin,
        ...
        self.create_placeholder(
            name="data", dtype=self.dtype, fill_value=0
        )  # sum of squared values
        self.create_placeholder(
            name="sum", dtype=self.dtype, fill_value=0
        )  # sum of values
        self.create_placeholder(name="counts", dtype=np.uint8, fill_value=0)
...
class WeightedSumAccumulator3D(Accumulator3D):
    ...
    def __init__(
        ...
        transform=None,
        path=None,
        weights_function=triangular_weights_function_nd,
        **kwargs,
    ):
        if dtype in [np.int8, np.uint8]:
            raise NotImplementedError(
                "`weighted` accumulation is unavailable for one-byte dtypes."
            )
        super().__init__(
            shape=shape,
            origin=origin,
    ...
    def _update(self, crop, location):
        ...
        self.data[location] = (
            self.crop_weights * crop + self.data[location] * self.weights[location]
...
class RegressionAccumulator(Accumulator3D):
    ...
    def __init__(
        ...
        path=None,
        weights_function=triangular_weights_function_nd,
        rsquared_lower_bound=0.2,
        regression_target="assembled",
        **kwargs,
    ):
        super().__init__(
            shape=shape,
    ...
    def _update(self, crop, location):
        ...
        if len(overlap_indices[0]) > 0:
            ...
            rsquared = (
                1 - ((a * xs + b - ys) ** 2).mean() / ((ys - ys.mean()) ** 2).mean()
            )
            ...
            if rsquared > self.rsquared_lower_bound:
                ...
                overlap_data[overlap_indices] = (
                    overlap_weights[overlap_indices] * overlap_data[overlap_indices]
