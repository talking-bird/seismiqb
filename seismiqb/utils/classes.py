...
class LoopedList(list):
    ...
    def __getitem__(self, idx):
        if idx >= len(self):
            ...
            if pos < 0:
                raise IndexError(
                    f"List of length {len(self)} is looped from {self.loop_from} index"
                )
...
class DelegatingList(AugmentedList):
    ...
    def __init__(self, obj=None):
        ...
        super().__init__(
            [type(self)(item) if isinstance(item, list) else item for item in obj]
        )
    ...
    def to_dict(self):
        ...
        for key in self.reference_object:
            try:
                ...
                raise ValueError(
                    f"KeyError occured due to absence of key `{key}` in some of list items."
                ) from e
...
class AugmentedDict(OrderedDict):
    ...
    def __getattr__(self, key):
        ...
        if not callable(attribute):
            ...
            return AugmentedDict(
                {key_: getattr(value, key) for key_, value in self.items()}
            )
        ...
        def method_wrapper(*args, **kwargs):
            return AugmentedDict(
                {
                    key_: getattr(value, key)(*args, **kwargs)
                    for key_, value in self.items()
                }
            )
    ...
    def flatten(self, keys=None):
        ...
        lists = [
            self[key] if isinstance(self[key], list) else [self[key]] for key in keys
        ]
        flattened = sum(lists, [])
