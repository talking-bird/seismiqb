...
class TransformsMixin:
    ...
    def matrix_dilate(matrix, dilation_iterations=3):
        ...
        dilated = cv2.dilate(
            matrix.astype(np.float32), kernel, iterations=dilation_iterations
        )
    ...
    def wrapper(
        ...
        **kwargs,
        ...
        if (dilation_iterations > 0) and hasattr(instance, "matrix_dilate"):
            result = instance.matrix_dilate(
                result, dilation_iterations=dilation_iterations
            )
        ...
        if n_components is not None and hasattr(instance, "pca_transform"):
            if result.ndim != 3:
                raise ValueError(
                    f"PCA transformation can be applied only to 3D arrays, got `{result.ndim}`"
                )
