...
class _GlobalCache:
    ...
    def get_size(self, level="total", format="dict"):
        ...
        result = (
            result["size"] if (level == "total" and format == "default") else result
        )
        return result
    ...
    def get_nbytes(self, level="total", format="dict"):
        ...
        result = (
            result["nbytes"] if (level == "total" and format == "default") else result
        )
        return result
    ...
    def get_stats(self, stats="size", level="total", format="default"):
        ...
        for instance in self.instances_with_cache:
            instance_stats = {
                stat: getattr(instance, f"cache_{stat}") for stat in stats
            }
            ...
            if level == "total":
                ...
                result[instance.__class__.__name__][f"id_{id(instance)}"] += (
                    instance_stats
                )
        ...
        if format == "dict":
            ...
            if level == "total":
                ...
                result = pd.concat(
                    {k: pd.DataFrame(v).T for k, v in result.items()}, axis=0
                )
    ...
    def get_cache_repr(self, format="dict"):
        ...
        if format == "df" and len(cache_repr_) > 0:
            ...
            cache_repr_ = cache_repr_.loc[
                :, ["size", "nbytes", "arguments"]
            ]  # Columns sort
...
class lru_cache:
    ...
    def __init__(
        self, maxsize=128, attributes=None, apply_by_default=True, copy_on_return=False
    ):
    ...
    def make_key(self, instance, func, args, kwargs):
        ...
        args_and_defaults = [
            name
            for name in self.func_signature.keys()
