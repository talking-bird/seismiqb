...
def to_device(array, device="cpu"):
    ...
    if isinstance(device, int):
        if CUPY_AVAILABLE:
            ...
            warn(
                "Performance Warning: computing metrics on CPU as `cupy` is not available",
                RuntimeWarning,
            )
    return array
...
class BaseMetrics:
    ...
    PLOT_DEFAULTS = {"cmap": "Metric", "mask_color": "black"}
        ...
        **kwargs,
        ...
        if visualize:
            ...
            if savepath is not None:
                plot_config["savepath"] = self.horizon.field.make_path(
                    savepath, name=self.name
                )
    ...
    def compute_local(
        ...
        for i in range(k):
            for j in range(-k + 1, k):
                ...
                shifted_bad_traces = padded_bad_traces[
                    k + i : k + i + i_range, k + j : k + j + x_range
                ]

                computed = function(data, shifted_data, data_stds, shifted_stds)
                # Using symmetry property:
                symmetric_bad_traces = padded_bad_traces[
                    k - i : k - i + i_range, k - j : k - j + x_range
                ]
                symmetric_computed = computed[
                    : i_range - i, max(0, -j) : min(x_range, x_range - j)
                ]
                symmetric_computed = xp.pad(
                    symmetric_computed,
                    ((i, 0), (max(0, j), -min(0, j))),
    ...
    def find_supports(
        ...
        if isinstance(supports, int):
            ...
            support_coords = xp.asarray(
                [valid_traces[0][indices], valid_traces[1][indices]]
            ).T
            ...
            raise TypeError(
                "Unknown type for the `supports` argument. "
                "It must be one of: int, tuple, list or np.ndarray."
            )

        if len(support_coords) == 0:
            raise ValueError(
                "No valid support coordinates was found. "
    ...
    def compute_support(
        ...
        support_coords = BaseMetrics.find_supports(
            supports=supports,
            bad_traces=bad_traces,
            safe_strip=safe_strip,
            carcass_mode=carcass_mode,
            horizon=horizon,
            device=device,
            seed=seed,
        )
        ...
        accumulator = Accumulator(
            agg=agg, amortize=amortize, axis=axis, total=len(support_traces)
        )
        ...
        for i, _ in enumerate(support_traces):
            computed = function(
                valid_data, support_traces[i], valid_stds, support_stds[i]
            )
            accumulator.update(computed)
            pbar.update()
        pbar.close()
        ...
        result = xp.full(
            shape=(data_n.shape[0], data_n.shape[1]),
            fill_value=xp.nan,
            dtype=data_n.dtype,
        )
    ...
    def local_corrs(
        ...
        **kwargs,
        ...
        title = (
            f"Local correlation, k={kernel_size}, with `{agg}` aggregation\nfor {title}"
        )
        plot_config = {
            **plot_defaults,
            "title": title,
...
class HorizonMetrics(BaseMetrics):
    ...
    def __init__(self, horizons, window=23, offset=0, normalize=False, chunk_size=256):
        ...
        self.window, self.offset, self.normalize, self.chunk_size = (
            window,
            offset,
            normalize,
            chunk_size,
        ...
        **kwargs,
        ...
        if isinstance(supports, int):
            ...
            for horizon in horizons:
                horizon_bad_traces = (horizon.full_matrix == horizon.FILL_VALUE).astype(
                    int
                )
        ...
        for horizon in horizons:
            horizon_metric = cls(horizon).evaluate(
                metric=metric, supports=support_coords, horizon=horizon, **kwargs
            )
    ...
    def data(self):
        ...
        if self._data is None:
            self._data = self.horizon.get_cube_values(
                window=self.window, offset=self.offset, chunk_size=self.chunk_size
            )
...
class FaciesMetrics:
    ...
    def evaluate(self, metrics):
        ...
        for horizon, true_label, pred_label in zip_longest(
            self.horizons, self.true_labels, self.pred_labels
        ):
            kwargs = {}
            ...
            index = pd.MultiIndex.from_arrays(
                [[horizon.field.short_name], [horizon.short_name]],
