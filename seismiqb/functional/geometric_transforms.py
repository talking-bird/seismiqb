...
def rotate_3d(array, angle, adjust=False, fill_value=0):
    ...
    if angle[1] != 0:
        array = _rotate_2d(array.transpose(1, 2, 0), angle[1], fill_value).transpose(
            2, 0, 1
        )
    if angle[2] != 0:
        array = _rotate_2d(array.transpose(2, 0, 1), angle[2], fill_value).transpose(
            1, 2, 0
        )
    return array
...
def _rotate_2d(array, angle, fill_value=0):
    ...
    return cv2.warpAffine(
        array, matrix, (shape[1], shape[0]), borderValue=fill_value
    ).reshape(shape)
...
def resize(array, size, interpolation=1):
    ...
    if array.shape[0] == 1:
        resized = cv2.resize(
            src=array.squeeze(), dsize=(size[1], size[0]), interpolation=interpolation
        )
        resized = resized.reshape(1, *resized.shape)
    else:
        resized = cv2.resize(
            src=array, dsize=(size[1], size[0]), interpolation=interpolation
        )
    return resized
...
def scale_3d(array, scale, adjust=False):
    ...
    array = _scale_2d(array, [scale[0], scale[1]])
    array = _scale_2d(array.transpose(1, 2, 0), [1, scale[-1]]).transpose(2, 0, 1)
    return array
...
def _scale_2d(array, scale):
    ...
    matrix[:, -1] = (
        np.array([shape[1], shape[0]]) * (1 - np.array([scale[1], scale[0]])) / 2
    )
...
def adjust_shape(array, angle=0, scale=(1, 1, 1)):
    ...
    array = cv2.resize(array, dsize=(adjusted_shape[1], adjusted_shape[0])).reshape(
        *adjusted_shape
    )
...
def center_crop(array, shape):
    ...
    if (new_shape > old_shape).any():
        raise ValueError(
            f"Output shape={new_shape} can't be larger than input shape={old_shape}!"
        )

    corner = old_shape // 2 - new_shape // 2
    slices = tuple(
        slice(start, start + length) for start, length in zip(corner, new_shape)
    )
    return array[slices]
...
def elastic_transform(array, alpha=40, sigma=4, rng=None):
    ...
    rand_x = (
        cv2.GaussianBlur(
            rng.random(size=grid_shape, dtype=np.float32) * 2 - 1,
