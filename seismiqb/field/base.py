...
class Field(CharismaMixin, VisualizationMixin):
    ...
    NAME_TO_METHOD = {
        name: method for method, names in METHOD_TO_NAMES.items() for name in names
    }
    ...
    def load_labels(self, labels=None, labels_class=None, mode="w", **labels_kwargs):
        ...
        if not isinstance(labels, dict):
            raise TypeError(
                f"Labels type should be `str`, `sequence` or `dict`, got {type(labels)} instead!"
            )
        ...
        for label_dst, label_src in labels.items():
            ...
            if label_class is None:
                ...
                str_names = [
                    name
                    for name in (self.NAME_TO_METHOD.keys())
    ...
    def _filter_paths(paths):
        ...
        return [
            ...
            or not any(
                ext in path for ext in [".dvc", ".gitignore", ".meta", ".sgy_meta"]
            )
        ]
    ...
    def _load_horizons(
        ...
        **kwargs,
        ...
        with ThreadPoolExecutor(
            max_workers=min(max_workers, len(paths_to_load) or 1)
        ) as executor:
            function = lambda path: self._load_horizon(
                path,
                filter=filter,
                interpolate=interpolate,
                constructor_class=label_class,
                **kwargs,
            )
    ...
    def _load_horizon(
        self, path, filter=True, interpolate=False, constructor_class=Horizon, **kwargs
    ):
    ...
    def _load_faults(
        ...
        **kwargs,
        ...
        with ThreadPoolExecutor(
            max_workers=min(max_workers, len(paths) or 1)
        ) as executor:
            function = lambda path: self._load_fault(
                path, interpolate=interpolate, constructor_class=label_class, **kwargs
            )
            loaded = list(
                Notifier(pbar, total=len(paths))(executor.map(function, paths))
            )
    ...
    def __getattr__(self, key):
        ...
        if (
            not key.endswith("state__")
            and "geometry" in self.__dict__
            and hasattr(self.geometry, key)
        ):
            return getattr(self.geometry, key)
        raise AttributeError(
            f"Attribute `{key}` does not exist in either Field or associated Geometry!"
        )
    ...
    def make_regression_mask(
        self, location, axis=None, indices="all", src="labels", **kwargs
    ):
        ...
        if len(labels) > 1:
            ...
            labels[0].add_to_regression_mask(
                mask=mask[..., 0], locations=location, **kwargs
            )
        return mask
    ...
    def load_attribute(self, src, _return_label=False, **kwargs):
        ...
        if any(sep in label_id for sep in ":-"):
            ...
            if label_attr not in self.loaded_labels:
                raise ValueError(
                    f"Can't determine the label attribute for `{label_attr}`!"
                )
            ...
            if isinstance(labels, list):
                for idx, label in enumerate(labels):
                    ...
                    available_names.extend(
                        [f"{name}:{idx}/{attr}" for attr in available_attributes]
                    )
            else:
                if isinstance(labels, Geometry):
                    available_attributes = ["mean_matrix", "std_matrix", "snr"]
                available_names.extend(
                    [f"{name}/{attr}" for attr in available_attributes]
                )
    ...
    def make_path(self, path, name=None, makedirs=True):
        ...
        path = path.replace("~", basedir).replace("$", name).replace("//", "/")
    ...
    def evaluate_facies(
        ...
        fm = FaciesMetrics(
            horizons=horizons, true_labels=true_labels, pred_labels=pred_labels
        )
