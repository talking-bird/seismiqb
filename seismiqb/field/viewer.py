...
class FieldViewer:
    ...
    def __init__(self, field, figsize=(8, 8)):
        ...
        self.attribute_dropdown = widgets.Dropdown(
            options=self.field.available_attributes,
            value=self.field.available_attributes[2],
            description="Base map",
            layout=widgets.Layout(max_width="350px"),
        )
        self.location_text = widgets.IntText(
            value=location,
            description="INLINE",
            layout=widgets.Layout(max_width="150px"),
        )
        self.location_slider = widgets.IntSlider(
            value=location,
            max=self.field.shape[0] - 1,
            description="INLINE",
            continuous_update=False,
            layout=widgets.Layout(min_width="500px"),
        )
        self.axis_button = widgets.Button(
            description="swap axis", layout=widgets.Layout(min_width="200px")
        )
        ...
        with widgets.Output():
            ...
            self.vbox = widgets.VBox([self.hbox, self.fig.canvas])
    ...
    def draw_slice(self, force=False):
        ...
        lines = [
            children
            for children in self.base_ax.get_children()
            if hasattr(children, "created_by_draw")
            or hasattr(children, "created_by_zoom")
        ]
        for line in lines:
            line.remove()
        ...
        if axis == 0:
            line = self.base_ax.vlines(
                location, 0, self.field.shape[1], color="r", linewidth=3
            )
        elif axis == 1:
            line = self.base_ax.hlines(
                location, 0, self.field.shape[0], color="r", linewidth=3
            )
