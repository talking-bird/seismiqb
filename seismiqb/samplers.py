...
class BaseSampler(Sampler):
    ...
    def _make_locations(
        self, field, points, matrix, crop_shape, ranges, threshold, filtering_matrix
    ):
        # Parse parameters
        ranges = ranges if ranges is not None else [None, None, None]
        ranges = [
            item if item is not None else [0, c] for item, c in zip(ranges, field.shape)
        ]
        ...
        i_mask = (ranges[:2, 0] <= points[:, :2]).all(axis=1) & (
            (points[:, :2] + crop_shape[:2]) <= ranges[:2, 1]
        ).all(axis=1)
        x_mask = (ranges[:2, 0] <= points[:, :2]).all(axis=1) & (
            (points[:, :2] + crop_shape_t[:2]) <= ranges[:2, 1]
        ).all(axis=1)
        mask = i_mask | x_mask
        ...
        if threshold != 0.0:
            points = spatial_check_points(
                points, matrix, crop_shape[:2], i_mask, x_mask, n_threshold
            )
        else:
...
class GeometrySampler(BaseSampler):
        ...
        **kwargs,
        ...
        points = np.hstack(
            [
                idx[0].reshape(-1, 1),
                idx[1].reshape(-1, 1),
                np.zeros((len(idx[0]), 1), dtype=np.int32),
            ]
        ).astype(np.int32)
    ...
    def __repr__(self):
        return (
            f"<GeometrySampler for {self.short_name}: "
...
class HorizonSampler(BaseSampler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if randomize_depth:
            randomize_depth = (
                randomize_depth if isinstance(randomize_depth, tuple) else (0.9, 0.1)
            )
    ...
    def sample(self, size):
        ...
        if self.threshold == 0.0:
            ...
            while accumulated < size:
                ...
                condition = spatial_check_sampled(
                    sampled, self.matrix, self.n_threshold
                )
    ...
    def _sample(self, size):
        ...
        if self.spatial_shift:
            ...
            shift_i = np.random.randint(
                low=-(shapes_i * self.spatial_shift[0][0]).astype(np.int32),
            ...
            shift_x = np.random.randint(
                low=-(shapes_x * self.spatial_shift[1][0]).astype(np.int32),
            ...
            np.clip(
                sampled[:, 1],
                0,
                self.field.shape[0] - self.crop_shape[0],
                out=sampled[:, 1],
            )
            np.clip(
                sampled[:, 4],
            ...
            np.clip(
                sampled[:, 2],
                0,
                self.field.shape[1] - self.crop_shape[1],
                out=sampled[:, 2],
            )
            np.clip(
                sampled[:, 5],
    ...
    def __repr__(self):
        return (
            f"<HorizonSampler for {self.short_name}: "
            f"crop_shape={tuple(self.crop_shape)}, threshold={self.threshold}, "
...
class FaultSampler(BaseSampler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.locations = self._make_locations(
            field, crop_shape, ranges, threshold, extend
        )
    ...
    def _make_locations(self, field, crop_shape, ranges, threshold, extend):
        ...
        i_mask = (ranges[:2, 0] <= nodes[:, :2]).all(axis=1) & (
            (nodes[:, :2] + crop_shape[:2]) <= ranges[:2, 1]
        ).all(axis=1)
        x_mask = (ranges[:2, 0] <= nodes[:, :2]).all(axis=1) & (
            (nodes[:, :2] + crop_shape_t[:2]) <= ranges[:2, 1]
        ).all(axis=1)
    ...
    def sample(self, size):
        ...
        while accumulated < size:
            ...
            condition = volumetric_check_sampled(
                sampled,
                self.points,
                self.crop_shape,
                self.crop_shape_t,
    ...
    def _sample(self, size):
        ...
        for mask, shape in zip([i_mask, x_mask], [self.crop_shape, self.crop_shape_t]):
            ...
            shift = np.random.randint(
                low=low, high=high, size=(mask.sum(), 3), dtype=np.int32
            )
            sampled[mask, 1:4] += shift
            sampled[mask, 4:] += shift

            sampled[mask, 1:4] = np.clip(
                sampled[mask, 1:4], 0, self.field.shape - shape
            )
            sampled[mask, 4:7] = np.clip(
                sampled[mask, 4:7], 0 + shape, self.field.shape
            )
        return sampled

    def __repr__(self):
        return (
            f"<FaultSampler for {self.short_name}: "
            f"crop_shape={tuple(self.crop_shape)}, threshold={self.threshold}>"
        )
...
class WellSampler(Sampler):
    ...
    def __init__(
        ...
        label_id=None,
        threshold=0.0,
        spatial_randomization=(0.0, 1.0),
        depth_randomization=(0.0, 0.0),
        **kwargs,
    ):
        self.well = well
        self.crop_shape = crop_shape
    ...
    def _make_locations(
        self, well, crop_shape, log, spatial_randomization, depth_randomization
    ):
        ...
        depth_ranges = [
            max(bbox[-1][0] - crop_shape[-1] * depth_randomization[0], 0),
            min(
                bbox[-1][1] + crop_shape[-1] * depth_randomization[1],
        ...
        # inline-oriented
        arange_i = np.arange(
            max(location[0] - crop_shape[0] * spatial_randomization[1] + 1, 0),
            location[0] - crop_shape[0] * spatial_randomization[0] + 1,
        )
        arange_x = np.arange(
            max(location[1] - crop_shape[1] * spatial_randomization[1] + 1, 0),
        ...
        # crossline-oriented
        arange_i = np.arange(
            max(location[0] - crop_shape_t[0] * spatial_randomization[1] + 1, 0),
            location[0] - crop_shape_t[0] * spatial_randomization[0] + 1,
        )
        arange_x = np.arange(
            max(location[1] - crop_shape_t[1] * spatial_randomization[1] + 1, 0),
    ...
    def sample(self, size):
        ...
        if self.threshold == 0.0 or self.well.vertical:
            ...
            while accumulated < size:
                ...
                condition = np.array(
                    [
                        self.well.compute_overlap_size(
                            mask_bbox=location[1:].reshape(2, 3).T, log=self.log
                        )
                        >= self.threshold
...
def spatial_check_sampled(locations, matrix, threshold):
    ...
    for i, (_, i_start, x_start, d_start, i_stop, x_stop, d_stop) in enumerate(
        locations
    ):
...
def volumetric_check_sampled(locations, points, crop_shape, crop_shape_t, threshold):
    ...
    if threshold > 0:
        for i, (
            ...
            mask_bbox = np.array(
                [[i_start, i_stop], [x_start, x_stop], [d_start, d_stop]],
                dtype=np.int32,
            )
...
class SeismicSampler(Sampler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for field_id, (field_name, list_labels) in enumerate(labels.items()):
            list_labels = (
                list_labels if isinstance(list_labels, (tuple, list)) else [list_labels]
            )
            ...
            # Unpack parameters
            crop_shape_ = (
                crop_shape[field_name] if isinstance(crop_shape, dict) else crop_shape
            )
            threshold_ = (
                threshold[field_name] if isinstance(threshold, dict) else threshold
            )
            filtering_matrix_ = (
                filtering_matrix[field_name]
                if isinstance(filtering_matrix, dict)
                else filtering_matrix
            )
            ...
            if len(set(label_classes)) != 1:
                raise ValueError(
                    f"Labels contain different classes, {set(label_classes)}!"
                )
            ...
            if uniform_labels:
                ...
                weights = np.array(
                    [
                        len(label) if hasattr(label, "__len__") else 1
        ...
        # Resulting sampler
        n_present_fields = sum(
            len(sampler_list) != 0 for sampler_list in samplers.values()
        )
        ...
        for (field_name, sampler_list), p, l in zip(
            samplers.items(), cube_proportions, labels_weights
        ):
    ...
    def __len__(self):
        return sum(
            len(sampler.locations)
            for sampler_list in self.samplers.values()
    ...
    def show_locations(self, savepath=None, plotter=plot, **kwargs):
        ...
        for samplers_list in self.samplers.values():
            ...
            data += [
                [sampler.orientation_matrix, field.dead_traces_matrix]
                for sampler in samplers_list
            ]
            title += [
                f"{field.short_name}: {sampler.short_name}" for sampler in samplers_list
            ]
    ...
    def show_sampled(
        self, n=10000, binary=False, savepath=None, plotter=plot, **kwargs
    ):
