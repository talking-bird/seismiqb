...
class SegyioLoader:
    ...
    def __init__(self, path, endian="big", strict=False, ignore_geometry=True):
        ...
        if endian not in self.ENDIANNESS_TO_SYMBOL:
            raise ValueError(
                f"Unknown endian {endian}, must be one of {self.ENDIANNESS_TO_SYMBOL}"
            )
        ...
        self.delay = self.file_handler.header[0].get(
        ...
        self.text = [
            self.file_handler.text[i] for i in range(1 + self.file_handler.ext_headers)
        ]
    ...
    def _infer_sample_interval(self):
        ...
        trace_sample_interval = self.file_handler.header[0][
        ...
        if len(union_sample_interval) != 1:
            raise ValueError(
                "Cannot infer sample interval from file headers: "
        ...
        return (
            union_sample_interval.pop() / 1000
    ...
    def load_trace(self, index, buffer, limits):
        ...
        self.file_handler.xfd.gettr(
            buffer, index, 1, 1, limits.start, limits.stop, limits.step, buffer.size
        )
    ...
    def make_chunk_iterator(
        self, chunk_size=None, n_chunks=None, limits=None, buffer=None
    ):
        ...
        if chunk_size is not None and n_chunks is not None:
            raise ValueError(
                "Only one of `chunk_size` and `n_chunks` should be provided!"
            )
        ...
        iterator = (
            {
                "data": self.load_traces(
                    list(range(start, end)), limits=limits, buffer=buffer
                ),
                "start": start,
                "end": end,
            }
    ...
    def chunk_iterator(self, chunk_size=None, n_chunks=None, limits=None, buffer=None):
        ...
        return self.make_chunk_iterator(
            chunk_size=chunk_size, n_chunks=n_chunks, limits=limits, buffer=buffer
        )[0]
