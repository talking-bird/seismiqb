...
class ExportMixin:
    ...
    def default_export_spec(
        ...
        *_step : int
        ...
        spec.ilines = np.arange(
            i_start, i_start + iline_step * array_like.shape[0], dtype=np.int32
        )
        spec.xlines = np.arange(
            x_start, x_start + xline_step * array_like.shape[1], dtype=np.int32
        )
        spec.samples = np.arange(array_like.shape[2], dtype=np.int32)

        # Additional matrices
        spec.cdp_x_matrix = np.tile(
            cdp_x_shift + cdp_x_step * spec.ilines.reshape(-1, 1), array_like.shape[1]
        )
        spec.cdp_y_matrix = np.tile(
            cdp_y_shift + cdp_y_step * spec.xlines.reshape(-1, 1), array_like.shape[0]
        ).T
        return spec
    ...
    def array_to_segy(
        ...
        **kwargs,
        ...
        # Select the spec
        if spec is None:
            spec = ExportMixin.default_export_spec(
                array_like=array_like, origin=origin, format=format, **kwargs
            )
        ...
        # Export the data
        if engine == "segyio":
            ExportMixin.array_to_segy_segyio(
                array_like=array_like, path=path, spec=spec, pbar=pbar
            )
        else:
            ExportMixin.array_to_segy_memmap(
                array_like=array_like,
                path=path,
                spec=spec,
                pbar=pbar,
                format=format,
                endian_symbol=endian_symbol,
                transform=transform,
    ...
    def export_array(
        ...
        ExportMixin.array_to_segy(
            array_like=array_like,
            path=path,
            spec=spec,
            pbar=pbar,
            zip_segy=zip_segy,
            remove_segy=remove_segy,
            engine=engine,
            format=format,
            transform=transform,
            endian_symbol=endian_symbol,
    ...
    def array_to_segy_segyio(array_like, path, spec, pbar="t"):
        ...
        with segyio.create(path, spec) as dst_file:
            ...
            notifier = Notifier(
                pbar, total=len(spec.ilines), desc=f"Writing `{basename}`"
            )
                    ...
                    dst_file.header[c].update(
                        {
                            ...
                            segyio.TraceField.TRACE_SAMPLE_INTERVAL: int(
                                spec.sample_interval * 1000
                            ),
    ...
    def array_to_segy_memmap(
        ...
        dst_dtype = (
            endian_symbol + MemmapLoader.SEGY_FORMAT_TO_TRACE_DATA_DTYPE[spec.format]
        )
        dst_itemsize = np.dtype(dst_dtype).itemsize
        dst_size = 3600 + n_traces * (
            MemmapLoader.TRACE_HEADER_SIZE + n_samples * dst_itemsize
        )
        ...
        dst_mmap[3217 - 1 : 3217 - 1 + 2] = np.array(
            [int(spec.sample_interval * 1000)], dtype=endian_symbol + "u2"
        ).view("u1")
        dst_mmap[3221 - 1 : 3221 - 1 + 2] = np.array(
            [n_samples], dtype=endian_symbol + "u2"
        ).view("u1")
        dst_mmap[3225 - 1 : 3225 - 1 + 2] = np.array(
            [spec.format], dtype=endian_symbol + "u2"
        ).view("u1")
        ...
        if os.name == "nt":
            mmap_trace_dtype = np.dtype(
                [
                    ("headers", np.uint8, MemmapLoader.TRACE_HEADER_SIZE),
                    ("data", dst_dtype, n_samples),
                ]
            )
            dst_mmap = np.memmap(
                path, mode="r+", offset=3600, shape=(n_traces,), dtype=mmap_trace_dtype
            )
            dst_mmap["headers"] = 0

        # Prepare the export dtype
        mmap_trace_headers_dtype = MemmapLoader._make_mmap_headers_dtype(
            (
                "INLINE_3D",
                "CROSSLINE_3D",
                "CDP_X",
        ...
        mmap_trace_dtype = np.dtype(
            [*mmap_trace_headers_dtype, ("data", dst_dtype, n_samples)]
        )
...
def write_chunk(
    path, shape, offset, dtype, spec, array_like, transform, start, chunk_size
):
    ...
    # Write trace headers
    dst_traces["INLINE_3D"] = np.repeat(
        spec.ilines[start : start + chunk_size], len(spec.xlines)
    )
