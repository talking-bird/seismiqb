...
class Geometry(
    ...
    PRESERVED_LAZY = [  # loaded at the time of the first access
        "index_unsorted_uniques",
        "index_sorted_uniques",
        "index_value_to_ordinal",
        "min_vector",
        "max_vector",
        "mean_vector",
        "std_vector",
    ...
    def __init__(
        self, path, meta_path=None, safe=False, use_cache=False, init=True, **kwargs
    ):
        # Path to the file
        self.path = path
    ...
    def __getattr__(self, key):
        ...
        if (
            ...
            and (key in self.PRESERVED_LAZY_ALL)
            and self.meta_storage.exists
            and self.meta_storage.has_item(key)
        ):
    ...
    def dump_meta(self, path=None):
        ...
        items = {
            ...
            for key in self.PRESERVED + self.PRESERVED_LAZY + self.PRESERVED_LAZY_MISC
    ...
    def __setstate__(self, state):
        ...
        if self.converted:
            ...
            self.loader = self._infer_loader_class(
                self._init_kwargs.get("loader_class", "memmap")
            )(self.path)
    ...
    def process_key(self, key):
        ...
        for i, (subkey, limit) in enumerate(zip(key, self.shape)):
            if isinstance(subkey, slice):
                slc = slice(
                    max(subkey.start or 0, 0),
                    min(subkey.stop or limit, limit),
                    subkey.step,
                )
    ...
    # Data loading: cache
    def load_slide(
        self, index, axis=0, limits=None, buffer=None, safe=None, use_cache=None
    ):
        ...
        if use_cache is False:
            return self.load_slide_native(
                index=index, axis=axis, limits=limits, buffer=buffer, safe=safe
            )
    ...
    def load_slide_cached(self, index, axis=0, limits=None):
        ...
        return self.load_slide_native(
            index=index, axis=axis, limits=limits, buffer=None, safe=True
        )
    ...
    def load_crop_cached(self, locations, axis=None, buffer=None):
        ...
        for i, idx in enumerate(range(locations[0].start, locations[0].stop)):
            buffer[i] = self.load_slide_cached(index=idx, axis=axis)[
    ...
    def load_subset(self, n_traces=100_000, seed=42):
        ...
        if self.converted is False:
            ...
            indices = rng.choice(
                self.shape[0], size=n_traces // self.shape[1], replace=False
            )
            data = []
    ...
    def lines_to_cdp(self, points):
        ...
        return (
            self.rotation_matrix[:, :2] @ points.T
            + self.rotation_matrix[:, 2].reshape(2, -1)
        ).T
    ...
    def quantile_interpolator(self):
        ...
        if self._quantile_interpolator is None:
            self._quantile_interpolator = interp1d(
                self.quantile_support, self.quantile_values
            )
    ...
    def make_normalizer(
        ...
        self._normalizer = Normalizer(
            mode=mode,
            clip_to_quantiles=clip_to_quantiles,
            q=q,
    ...
    def make_quantization_stats(
        ...
        self._quantization_stats = self.compute_quantization_parameters(
            ranges=ranges,
            clip=clip,
            center=center,
            dtype=dtype,
            n_quantile_traces=n_quantile_traces,
            seed=seed,
        )
    ...
    def estimate_impulse(self, wavelet_length=40, n_traces=10_000, seed=42):
        ...
        wavelet_estimation = np.concatenate(
            (wavelet_estimation[1:][::-1], wavelet_estimation), axis=0
        )
    ...
    def nbytes(self):
        ...
        return self.cache_size + sum(
            sys.getsizeof(getattr(self, attribute))
    ...
    def print_location(self, printer=print):
        ...
        msg = "\n".join(
            f"{header + ':':<35} [{uniques[0]}, {uniques[-1]}]"
    ...
    def show(self, matrix="snr", plotter=plot, **kwargs):
        ...
        matrix_name = (
            ...
            else kwargs.get("matrix_name", "custom matrix")
        )
        kwargs = {
            "cmap": "magma",
    ...
    def show_histogram(
        self, n_traces=100_000, seed=42, bins=50, plotter=plot, **kwargs
    ):
    ...
    def show_spectrum(
        ...
        **kwargs,
        ...
        power_spectrum_smoothed = savgol_filter(
            power_spectrum, filter_length, filter_order
        )
        f1, f2 = frequencies[
            [np.argmax(power_spectrum), np.argmax(power_spectrum_smoothed)]
        ]
        ...
        kwargs = {
            ...
            "label": [
                [
                    f"power spectrum: max at {f1:3.1f} Hz",
                    f"power spectrum smoothed: max at {f2:3.1f} Hz",
                ],
                "",
            ],
            **kwargs,
        ...
        plotter = plot(
            [
                [(frequencies, power_spectrum), (frequencies, power_spectrum_smoothed)],
    ...
    def show_section(
        ...
        **kwargs,
        ...
        nearest_ticks = np.argmin(
            np.abs(xticks.reshape(-1, 1) - nodes.reshape(1, -1)), axis=0
        )
        xticks[nearest_ticks] = nodes
        labels = np.array(list(map("\n".join, indices.astype("int32").astype(str))))[
            xticks % section.shape[0]
        ]
    ...
    def show_section_map(
        ...
        **kwargs,
        ...
        plotter[0].ax.scatter(
            locations[:, 0], locations[:, 1], c=pointcolor, s=pointsize, marker=marker
        )
        plotter[0].ax.plot(
            locations[:, 0], locations[:, 1], color=linecolor, linewidth=linewidth
        )
    ...
    def get_slide_index(self, index, axis=0):
        ...
        if isinstance(index, (int, np.integer)):
            if index >= self.shape[axis]:
                raise KeyError(
                    f"Index={index} is out of geometry bounds={self.shape[axis]}!"
                )
            return index
        if index == "random":
    ...
    def get_slide_bounds(self, index, axis=0):
        ...
        right_bound = len(dead_traces) - np.argmin(
            dead_traces[::-1]
        )  # the first dead trace
    ...
    def get_slide_mask(
        self, index, axis=0, kernel_size=9, threshold=None, erosion=11, dilation=60
    ):
        ...
        locations[axis] = slice(index, index + 1)
        return self.get_crop_mask(
            tuple(locations), axis, kernel_size, threshold, erosion, dilation
        )

    def get_crop_mask(
        self, locations, axis=0, kernel_size=9, threshold=None, erosion=11, dilation=60
    ):
        ...
        erosion_kernel = (
            np.ones((erosion, erosion), dtype=array.dtype) if erosion else None
        )
        ...
        for i in range(array.shape[axis]):
            ...
            if erosion:
                mask_ = binary_erosion(
                    mask_, structure=erosion_kernel, border_value=True
                )
            if dilation:
                mask_ = cv2.dilate(mask_.astype("uint8"), dilation_kernel).astype(
                    "bool"
                )
            slc = [slice(None)] * 3
            slc[axis] = i
