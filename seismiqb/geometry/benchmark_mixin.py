...
class BenchmarkMixin:
    ...
    def make_random_crop_locations(bounds, size_min=10, size_max=100, rng=None):
        ...
        locations = [
            slice(start, np.clip(start + size, bound_min, bound_max))
            for start, size, bound_min, bound_max in zip(
                point, shape, bounds[0], bounds[1]
            )
        ]
        return locations
    ...
    def benchmark(
        array_like,
        n_slides=300,
        slide_allowed_axis=(0, 1, 2),
        n_crops=300,
        crop_size_min=(10, 10, 256),
        crop_size_max=(128, 128, 512),
        seed=42,
        pbar=False,
    ):
        ...
        if n_slides:
            ...
            for _ in Notifier(pbar, desc="Slides benchmark")(range(n_slides)):
                slide_locations = BenchmarkMixin.make_random_slide_locations(
                    bounds=bbox, rng=rng, allowed_axis=slide_allowed_axis
                )
        ...
        if n_crops:
            ...
            for _ in Notifier(pbar, desc="Crops benchmark")(range(n_crops)):
                crop_locations = BenchmarkMixin.make_random_crop_locations(
                    bbox.T, rng=rng, size_min=crop_size_min, size_max=crop_size_max
                )
