...
class WellSeismicMatcher:
    ...
    def __init__(self, well, field, coordinates):
        ...
        self.field = field
        self.seismic_times = (
            np.arange(0, field.depth, dtype=np.float32) * field.sample_interval * 1e-3
        )  # seconds
    ...
    def process_well(
        ...
        if filter_dt:
            ...
            dt_values = self.well._compute_filtered_log(
                dt_values, **filtration_parameters
            )
        ...
        if impedance_log is not None:
            ...
            if filter_rhob:
                ...
                rhob_values = self.well._compute_filtered_log(
                    rhob_values, **filtration_parameters
                )
                self.well["RHOB_FILTERED"] = rhob_values

            # Recomputed AI. Can omit unit conversions as they dont influence the reflectivity
            self.well["AI_RECOMPUTED"] = rhob_values / (
                dt_values * 1e-6 / 0.3048
            )  # kPa.s/m
        ...
        if filter_ai:
            ...
            self.well["AI_FILTERED"] = self.well._compute_filtered_log(
                self.well[impedance_log], **filtration_parameters
            )
    ...
    def extract_wavelet(
        ...
        **kwargs,
        ...
        k0, k1 = window
        traces = self.field.geometry[
            c0 - k0 // 2 : c0 + (k0 - k0 // 2), c1 - k1 // 2 : c1 + (k1 - k1 // 2)
        ]
        ...
        if method in {"lstsq", "division"}:
            ...
            reflectivity = self.resample_to_seismic(
                seismic_times=self.seismic_times,
        ...
        if method == "deterministic":
            ...
            if method in {"stats1", "statistical"}:
                ...
                autocorrelation = fftconvolve(
                    traces, traces[:, ::-1], mode="same", axes=-1
                )
                autocorrelation = autocorrelation[
                    :, lenhalf - wlenhalf : lenhalf + wlenhalf + wlenflag
                ]
                power_spectrum = np.sqrt(
                    np.abs(np.fft.rfft(autocorrelation, axis=-1))
                ).mean(axis=0)
            ...
            if smoothing:
                ...
                power_spectrum = savgol_filter(
                    power_spectrum, smoothing_length, smoothing_order
                )
    ...
    # Helper functions
    def compute_resampled_synthetic(
        self, well_times=None, wavelet=None, limits=None, multiply=False, **kwargs
    ):
        ...
        reflectivity_resampled = self.resample_to_seismic(
            seismic_times=self.seismic_times[limits],
            well_times=well_times,
            well_data=self.well_reflectivity,
        )
        synthetic_trace = self.well._compute_synthetic(
            reflectivity_resampled, wavelet=wavelet
        )

        if multiply:
            synthetic_trace *= self.compute_multiplier(
                self.seismic_trace[limits], synthetic_trace
            )
    ...
    def compute_metric(
        ...
        **kwargs,
        ...
        if synthetic_trace is None:
            synthetic_trace = self.compute_resampled_synthetic(
                well_times=well_times, wavelet=wavelet, limits=limits
            )
    ...
    def correlation(array_0, array_1):
        ...
        return ((array_0 - array_0.mean()) * (array_1 - array_1.mean())).mean() / (
            array_0.std() * array_1.std()
        )
    ...
    def compute_t0(self, ranges=(-0.5, +1.5), n=1000, limits=None, state=-1, index=0):
        ...
        values = [
            self.compute_metric(
                well_times=well_times + shift, wavelet=wavelet, limits=limits
            )
            for shift in shifts
        ]
    ...
    def optimize_t0(self, state=-1, limits=None, **kwargs):
        ...
        def minimization_proxy(x):
            return -self.compute_metric(
                well_times=self.well_times + x, wavelet=wavelet, limits=limits
            )
    ...
    @staticmethod
    def stretch_well_times(
        well_times, position, alpha, left_bound, right_bound, **kwargs
    ):
    ...
    def optimize_extrema(
        self,
        topk=20,
        threshold_max=0.050,
        threshold_min=0.001,
        threshold_nearest=0.010,
        threshold_iv_max=500,
        alpha_bounds=(0.9, 1.1),
        state=-1,
        **kwargs,
    ):
        ...
        peak_indices = peak_indices[
            np.argsort(values[peak_indices])[::-1]
        ]  # sort peaks by extrema value
        ...
        for index in range(min(topk, len(peak_indices))):
            ...
            if (
                min(
                    abs(well_times[peak_position] - well_times[left_bound]),
                    abs(well_times[peak_position] - well_times[right_bound]),
                )
                <= threshold_nearest
            ):
                continue
            ...
            optimization_results = minimize(
                minimization_proxy, x0=1.0, bounds=[optimization_bounds], **kwargs
            )
            ...
            beta = 1 + (1 - alpha) * xsum / ysum
            if (
                (1 - threshold_min / xsum) <= alpha <= (1 + threshold_min / ysum)
                or (1 - threshold_min / xsum) <= beta <= (1 + threshold_min / ysum)
                or beta <= alpha_bounds[0]
                or beta >= alpha_bounds[1]
            ):
                continue
            ...
            if (
                np.abs(iv_diffs_x).max() > threshold_iv_max
                or np.abs(iv_diffs_y).max() > threshold_iv_max
            ):
                continue
        ...
        state.update(
            {
                ...
                "time_before": well_times[position],
                "time_after": new_well_times[position],
                "correlation_delta": state["correlation"]
                - previous_state["correlation"],
    ...
    def optimize_extremas(
        ...
        **kwargs,
        ...
        for i in range(steps):
            success = self.optimize_extrema(
                topk=topk,
                threshold_max=threshold_max,
                threshold_min=threshold_min,
                threshold_nearest=threshold_nearest,
                alpha_bounds=alpha_bounds,
                threshold_iv_max=threshold_iv_max,
                **kwargs,
            )
            if not success:
                if verbose:
            ...
            if verbose:
                ...
                print(
                    f"{i:3} :: {correlation=:3.5f} :: {correlation_delta=:3.5f} :: {time_shift=:>+7.4} ms"
    ...
    @staticmethod
    def compute_resampled_synthetic_pytorch(
        well_times, well_reflectivity, seismic_times, wavelet
    ):
        """Compute synthetic trace in seismic times.
        ...
        reflectivity_resampled = Interp1D(
            well_times,
            well_reflectivity,
            method="linear",
            assume_sorted=True,
        ...
        synthetic_trace = torch.nn.functional.conv1d(
            input=reflectivity_resampled.reshape(1, 1, -1),
            weight=wavelet.reshape(1, 1, -1),
            padding="same",
        )
        return synthetic_trace.reshape(-1)

    def optimize_well_times_pytorch(
        self,
        n_segments=100,
        n_iters=1000,
        ...
        if n_segments == len(well_times) or n_segments == "well":
            multipliers = torch.ones(
                len(well_times), dtype=torch.float32, requires_grad=True
            )
            segment_size = 1
        else:
            ...
            multipliers = torch.ones(
                n_segments, dtype=torch.float32, requires_grad=True
            )
        ...
        well_reflectivity = (
            torch.from_numpy(np.nan_to_num(self.well_reflectivity)).float().clone()
        )
        ...
        optimizer_params = {"lr": 0.0002, **(optimizer_params or {})}
        ...
        notifier = Notifier(
            pbar,
            frequency=min(50, n_iters),
            monitors=[{"source": loss_history, "format": "correlation={:5.4f}"}],
        )
        for _ in notifier(n_iters):
            multipliers_ = torch.repeat_interleave(multipliers, segment_size)[
                : len(well_times)
            ]
            multipliers_[0] = 1.0
            new_well_times = torch.cumsum(dt * multipliers_, dim=0)
            synthetic_trace = self.compute_resampled_synthetic_pytorch(
                well_times=new_well_times,
        ...
        correlation = self.compute_metric(
            well_times=new_well_times, wavelet=previous_state["wavelet"]
        )
        state = {
    ...
    def optimize_wavelet(self, limits=None, state=-1, **kwargs):
        ...
        def minimization_proxy(phase_shift):
            new_wavelet = np.fft.irfft(
                spectrum * np.exp(1.0j * phase_shift), n=len(wavelet)
            )
            return -self.compute_metric(
                well_times=well_times, wavelet=new_wavelet, limits=limits
            )

        optimization_results = minimize(
            minimization_proxy, x0=0.0, bounds=[[-np.pi / 2, +np.pi / 2]], **kwargs
        )
    ...
    def optimize_wavelet_(self, n=10, delta=1.0, limits=None, state=-1, **kwargs):
        ...
        def minimization_proxy(phase_shifts):
            ...
            new_wavelet = np.fft.irfft(
                power_spectrum * np.exp(1.0j * new_phase_spectrum), n=len(wavelet)
            )
            return -self.compute_metric(
                well_times=well_times, wavelet=new_wavelet, limits=limits
            )
        ...
        optimization_results = minimize(
            minimization_proxy, x0=x0, bounds=optimization_bounds, **kwargs
        )
    ...
    def evaluate_markers(self, markers, state=-1):
        ...
        for idx, row in markers.iterrows():
            ...
            if (
                self.well.index[idx] - marker_depth
                > marker_depth - self.well.index[idx - 1]
            ):
                idx -= 1
    ...
    def save_las(self, path, state=-1):
        ...
        if well_times.size != self.well.shape[0]:
            pad_width = (
                self.well_bounds.start,
                self.well.shape[0] - self.well_bounds.stop,
            )
            dt_optimized = np.pad(
                dt_optimized, pad_width=pad_width, constant_values=np.nan
            )
            well_impedance = np.pad(
                well_impedance, pad_width=pad_width, constant_values=np.nan
            )

        lasfile = deepcopy(self.well.lasfile)
        lasfile.append_curve(
            "DT_OPTIMIZED", dt_optimized, unit="us/ft", descr="DT_OPTIMIZED"
        )
        ...
        if "RHOB_FILTERED" in self.well.keys:
            ...
            if well_times.size != self.well.shape[0]:
                rhob_values = np.pad(
                    rhob_values, pad_width=pad_width, constant_values=np.nan
                )
            lasfile.append_curve(
                "RHOB_FILTERED", rhob_values, unit="kg/m^3", descr="RHOB_FILTERED"
            )
    ...
    def save_synthetic(self, path, state=-1):
        ...
        array_to_segy(
            synthetic_trace, path=path, origin=(*self.coordinates, 0), pbar=False
        )
    ...
    def show_state(self, state=-1, limits=slice(None), force_dt=False, **kwargs):
        ...
        data = [
            [
                (seismic_times, self.seismic_trace[limits]),
                (seismic_times, synthetic_trace[limits]),
            ],
        ...
        if not np.allclose(dt, dt_state) or force_dt:
            ...
            data.append(
                [(well_times, iv), (well_times, iv_state), (well_times, iv_diff)]
            )
        ...
        kwargs = {
            ...
            "title": [
                "seismic vs synthetic",
                "wavelet",
                "interval velocity",
                "relative increase in velocity: dt/dt_state",
            ],
            "xlabel": ["seismic time, s", "time, s", "well time, s", "well time, s"],
            "ylabel": ["amplitude", "amplitude", "velocity, m/s", "ratio"],
            "label": [
                ["seismic_trace", "synthetic_trace"],
                "",
        ...
        if len(data) == 4:
            plotter.subplots[-1].ax.axhline(
                1, linestyle="dashed", alpha=0.5, color="sandybrown", linewidth=3
            )
    ...
    def show_progress(self, start_idx=1, **kwargs):
        ...
        data = [
            [
                state.get("correlation", self.compute_metric(**state))
                for state in self.states[start_idx:]
            ]
        ]
    ...
    def show_crosscorrelation(self, state=1, n_peaks=3, **kwargs):
        ...
        plotter[0].ax.scatter(
            state["peak_shifts"], state["peak_values"], s=15, c="r", marker="8"
        )

        for idx in range(n_peaks):
            shift = state["peak_shifts"][idx]
            correlation = state["peak_values"][idx]
            plotter[0].ax.axvline(
                shift,
                correlation,
                linestyle="dashed",
                color="orange",
    ...
    def show_time_shifts(self, zoom=None, **kwargs):
        ...
        plotter = plot(
            (self.seismic_times, self.seismic_times * np.nan), mode="curve", **kwargs
        )
        ...
        for i, state in enumerate(states):
            ...
            plotter[0].ax.annotate(
                text, xy=(max(time_before, time_after), (i + 1) / len(states)), size=14
            )
        return plotter
