...
class Intersection2d2d:
    ...
    def new(
        ...
        overlap = (
            np.maximum(bbox_0[:, 0], bbox_1[:, 0]),
            np.minimum(bbox_0[:, 1], bbox_1[:, 1]),
        )
        ...
        for point in points:
            ...
            instance = cls(
                field_0=field_0,
                field_1=field_1,
                trace_idx_0=trace_idx_0,
                trace_idx_1=trace_idx_1,
                limits=limits,
                pad_width=pad_width,
            )

            if not any(
                other.is_similar(instance, threshold=threshold) for other in result
            ):
    ...
    def adjust_index(field, trace_idx, n=3, use_std=False):
        ...
        nhalf = (n - 1) // 2
        indices = list(
            range(max(0, trace_idx - nhalf), min(trace_idx + nhalf + 1, field.n_traces))
        )
    ...
    def __init__(
        ...
        self.max_depth = int(
            max(
                field_0.depth * field_0.sample_interval + field_0.delay,
        ...
        self.distance = (
            (self.coordinates_0 - self.coordinates_1).astype(np.float64) ** 2
        ).sum() ** (1 / 2)
    ...
    def is_similar(self, other, threshold=10):
        ...
        if (
            abs(self.trace_idx_0 - other.trace_idx_0) <= threshold
            and abs(self.trace_idx_1 - other.trace_idx_1) <= threshold
        ):
            return True
        return False
    ...
    # Data
    def prepare_traces(
        self, limits=None, index_shifts=(0, 0), pad_width=None, n=1, transform=None
    ):
        ...
        trace_0 = self._prepare_trace(
            self.field_0,
            index=self.trace_idx_0 + index_shifts[0],
            limits=limits,
            pad_width=pad_width,
            n=n,
            transform=transform,
        )
        trace_1 = self._prepare_trace(
            self.field_1,
            index=self.trace_idx_1 + index_shifts[1],
            limits=limits,
            pad_width=pad_width,
            n=n,
            transform=transform,
    ...
    def _prepare_trace(
        self, field, index, limits=None, pad_width=None, n=1, transform=None
    ):
        # TODO: add taper
        # Load data
        nhalf = (n - 1) // 2
    ...
    def prepare_horizons(self):
        ...
        return (
            self._prepare_horizons(field=self.field_0, coordinates=self.coordinates_0),
    ...
    def match_traces_optimize(
        ...
        n=1,
        transform=None,
        init_shifts=range(-100, +100),
        init_angles=(0,),
        metric="r2",
        bounds_shift=(-150, +150),
        bounds_angle=None,
        bounds_gain=(0.9, 1.1),
        maxiter=100,
        eps=1e-6,
        **kwargs,
        ...
        # Load data
        trace_0, trace_1 = self.prepare_traces(
            limits=limits,
    ...
    def match_traces_analytic(
        ...
        **kwargs,
        ...
        if twostep:
            ...
            shifts = np.linspace(
                -max_shift, max_shift, 2 * max_shift + 1, dtype=np.float32
            )
            shift = self._match_traces_analytic(
                trace_0=trace_0,
                trace_1=trace_1,
                shifts=shifts,
                taper=taper,
            ...
            shifts = np.linspace(
                shift - twostep_margin,
                shift + twostep_margin,
                2 * twostep_margin * resample_factor + 1,
                dtype=np.float32,
            )
        else:
            shifts = np.linspace(
                -max_shift,
                max_shift,
                2 * max_shift * resample_factor + 1,
                dtype=np.float32,
            )

        # Compute `shift` with required precision
        matching_results = self._match_traces_analytic(
            trace_0=trace_0,
            trace_1=trace_1,
            shifts=shifts,
            taper=taper,
        ...
        matching_results["corr"] = self.evaluate(
    ...
    def _match_traces_analytic(
        ...
        metrics = (trace_0 * shifted_traces).mean(axis=1) / (
            trace_0.std() * shifted_traces.std(axis=1)
        )
        ...
        if apply_correction is False:
            ...
            correction = (
                metrics[idx - correction_step] - metrics[idx + correction_step]
            ) / (
                2 * metrics[idx - correction_step]
                - 4 * metrics[idx]
                + 2 * metrics[idx + correction_step]
            )
            quality = metrics[idx] - 0.25 * (
                (
                    (metrics[idx - correction_step] - metrics[idx + correction_step])
                    * correction
                )
            ...
            p1 = (
                ...
                if correction >= 0
                else instantaneous_phase[idx - correction_step]
            )
            if p1 - p0 > 180:
                p1 = p1 - 360
            elif p1 - p0 < -180:
                p1 = p1 + 360
            angle = p0 + (
                (p1 - p0) * correction if correction >= 0 else (p0 - p1) * correction
            )
    ...
    def match_on_horizon(self, horizon_name=None, **kwargs):
        ...
        if horizon_name is None:
            common_horizons = set(horizon_to_depth_0.keys()) & set(
                horizon_to_depth_1.keys()
            )
        ...
        return {"shift": shift, "angle": angle, "gain": gain, "corr": corr}
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        trace_0, trace_1 = self.prepare_traces(
            pad_width=pad_width, limits=limits, n=n, transform=transform
        )
        metric_function = compute_correlation if metric == "correlation" else compute_r2
        return metric_function(
            trace_0, modify_trace(trace_1, shift=shift, angle=angle, gain=gain)
        )
    ...
    def __repr__(self):
        return (
            f'<Intersection of "{self.field_0.short_name}.sgy"'
    ...
    def show_curves(
        ...
        **kwargs,
        ...
        matching_results = self.match_traces(
            method=method,
            limits=limits,
            index_shifts=index_shifts,
            pad_width=pad_width,
        ...
        shifts, metrics, envelope, instantaneous_phase = (
            matching_results["shifts"],
        ...
        kwargs = {
            "title": [
                f'traces of "{self.field_0.short_name}.sgy" x "{self.field_1.short_name}.sgy"'
            ...
            "label": [
                ["trace_0", "trace_1"],
                ["crosscorrelation", "envelope"],
                "instant phases",
            ],
            "xlabel": ["depth", "shift", "shift"],
            "xlabel_size": 16,
            "ylabel": ["amplitude", "metric", "phase (degrees)"],
            "xlim": [
                (start_tick, start_tick + len(trace_0)),
                (-max_shift, +max_shift),
                (-max_shift, +max_shift),
            ],
            "ratio": 0.8,
        ...
        if n_plots >= 2:
            ...
            plotter[1].add_legend(
                mode="curve",
                label=f"optimal shift: {shift:4.3f}",
                alpha=0.9,
                color="green",
            )
            ...
            plotter[1].add_legend(
                mode="curve",
                label=f"max correlation: {corr:4.3f}",
                alpha=0.9,
                color="red",
            )
        if n_plots >= 3:
    ...
    def show_lines(
        ...
        # Annotations
        ax.set_title(
            f'"{self.field_0.short_name}.sgy" and "{self.field_1.short_name}.sgy"',
            fontsize=26,
        )
    ...
    def show_metric_surface(
        ...
        n=1,
        transform=None,
        shifts=range(-20, +20 + 1, 1),
        angles=range(-180, +180 + 1, 30),
        figsize=(14, 8),
        cmap="seismic",
        levels=7,
        grid=True,
        ...
        contours = ax.contour(
            angles, shifts, metric_matrix, levels=levels, colors="k", linewidths=0.4
        )
        ax.clabel(
            contours,
            contours.levels,
            inline=True,
            fmt=lambda x: f"{x:2.1f}",
            fontsize=10,
        )
    ...
    def show_neighborhood(
        ...
        for i, index_shift_1 in enumerate(iterator):
            for j, index_shift_2 in enumerate(iterator):
                matching_results = self.match_traces_analytic(
                    index_shifts=(index_shift_1, index_shift_2),
                    limits=limits,
                    pad_width=pad_width,
                    n=n,
                    transform=transform,
    ...
    def show_composite_slide(
        ...
        **kwargs,
        ...
        # Make combined slide
        combined_slide, slide_0, slide_1 = self._prepare_combined_slide(
            sides=sides,
            data="field",
            shift=shift,
            angle=angle,
        ...
        mask_slide = self._prepare_combined_slide(
            sides=sides,
            data="labels",
            shift=shift,
            angle=0,
        ...
        extent = (
            0,
            combined_slide.shape[0],
            start_tick + combined_slide.shape[1],
            start_tick,
        )

        title = (
            f'"{self.field_0.short_name}.sgy":{sides[0]} x "{self.field_1.short_name}.sgy":{sides[1]}\n'
    ...
    def _prepare_slide(
        ...
        arange_ms = np.arange(
            slide.shape[1], step=(1 / field.sample_interval), dtype=np.float32
        )
        arange_ms += field.delay / field.sample_interval if data != "field" else 0
        interpolator = lambda trace: np.interp(
            arange_ms, arange, trace, left=0, right=0
        )
        slide = np.apply_along_axis(interpolator, 1, slide)

        # Pad to adjust for field delays
        slide = (
            np.pad(slide, ((0, 0), (field.delay, 0)))
            if field.delay > 0
            else slide[:, -field.delay :]
        )
    ...
    def compare_methods(
        ...
        **kwargs,
        ...
        """!!."""
        if getattr(self.field_0, "correction_results", None) and getattr(
            self.field_1, "correction_results", None
        ):
            n_plots = 4
        else:
            n_plots = 3
        ...
        if n_plots == 4:
            ...
            self.show_composite_slide(
                ...
                transform=transform,
                **matching_dict,
                title_prefix="method=`corrections`",
                width=width,
                axes=ax[3],
                adjust_figsize=False,
                **kwargs,
