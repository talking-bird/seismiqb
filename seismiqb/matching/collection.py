...
class FieldCollection:
    ...
    DEFAULT_GEOMETRY_KWARGS = {
        ...
        "collect_stats": False,
        "collect_stats_params": {"pbar": False},
        "dump_headers": True,
        "dump_meta": True,
    }
    ...
    def load_fields(self, fields, geometry_kwargs=None):
        ...
        geometry_kwargs = (
            ...
            else self.DEFAULT_GEOMETRY_KWARGS
        )
        return [
            (
                Field(item, geometry_kwargs=geometry_kwargs)
    ...
    def compute_intersections(
        self, limits=slice(None), pad_width=0, threshold=10, n_intersections=np.inf
    ):
            ...
            if add_instances:  # TODO: refactor
                ...
                points = np.array(
                    [
                        ...
                        indices,
                        np.round(depths).astype(np.int32),
                    ]
                ).T
    ...
    def find_intersection(self, name_0, name_1):
        ...
        for intersection in self.intersections.values():
            if (
                name_0 in intersection.field_0.name
                and name_1 in intersection.field_1.name
            ) or (
                name_1 in intersection.field_0.name
                and name_0 in intersection.field_1.name
            ):
                return intersection
    ...
    def get_matched_value(self, key):
        ...
        return [
            intersection.matching_results[key]
            for intersection in self.intersections.values()
        ]
    ...
    def intersections_df(self, errors=False, corrections=False, indices=False):
        ...
        if self.corrections:
            ...
            suspicious = (
                (shifts_errors > shifts_errors.mean() + 3 * shifts_errors.std())
    ...
    def compute_horizon_metric(self, horizon_name=None):
        ...
        metrics = []
        for key, intersection in self.intersections.items():
            metric_0 = intersection.compute_horizon_metric(
                horizon_name=horizon_name, shift=0
            )
            metric_1 = intersection.compute_horizon_metric(
                horizon_name=horizon_name, shift=shifts[key[0]] - shifts[key[1]]
            )
            metric_2 = intersection.compute_horizon_metric(
                horizon_name=horizon_name, shift=intersection.matching_results["shift"]
            )
        ...
        names = [
            "mean_horizon_shift",
            "mean_horizon_to_correction_shift",
            "mean_horizon_to_intersection_shift",
        ]
    ...
    def distribute_corrections(
        self, skip_index=-1, max_iters=100, alpha=0.75, tolerance=0.00001
    ):
    ...
    def remove_intersections(
        self, indices=None, skip_index=-1, max_iters=100, alpha=0.75, tolerance=0.00001
    ):
        ...
        self.distribute_corrections(
            skip_index=skip_index, max_iters=max_iters, alpha=alpha, tolerance=tolerance
        )
        return indices
    ...
    def fields_df(self):
        ...
        for i, field in enumerate(self.fields):
            ...
            for key, intersection in self.intersections.items():
                if i in key[:2]:
                    recomputed_corr = intersection.evaluate(
            ...
            if intersections:
                min_, mean_, std_ = (
                    np.min(recomputed_corrs),
                    np.mean(recomputed_corrs),
                    np.std(recomputed_corrs),
                )
                ...
                min_ = mean_ = std_ = mean_intersection = std_intersection = np.float64(
                    -1
                )
        ...
        columns = [
            ...
            "gain",
            "n_bad_intersections",
            "mean_recomputed_corr",
            "min_recomputed_corr",  # 'std_recomputed_corr',
            "n_intersections",
            "mean_corr_intersections",  # 'std_corr_intersections'
        ]
    ...
    @staticmethod
    def _export_segy(
        field, path, method="traces", apply_angle=True, apply_gain=True, pad_width=10
    ):
        ...
        mmap_trace_headers_dtype = dst_loader._make_mmap_headers_dtype(
            ["DelayRecordingTime"], endian_symbol=dst_loader.endian_symbol
        )
        mmap_trace_dtype = np.dtype(
            [
                *mmap_trace_headers_dtype,
                (
                    "data",
                    dst_loader.mmap_trace_data_dtype,
                    dst_loader.mmap_trace_data_size,
                ),
            ]
        )
        ...
        mmap = np.memmap(
            filename=path,
            mode="r+",
            shape=dst_loader.n_traces,
            offset=dst_loader.file_traces_offset,
        ...
        if method == "headers":
            ...
            arange_ms = np.arange(
                data.shape[1], step=(1 / field.sample_interval), dtype=np.float32
            )
    ...
    def _copy_segy(field, path):
        ...
        src_mmap = np.memmap(
            field.path,
            mode="r",
            shape=src_loader.n_traces,
            offset=src_loader.file_traces_offset,
            dtype=src_loader.mmap_trace_dtype,
        )
    ...
    def export_horizon(self, path, horizon_name=None, encoding=None):
        ...
        for field in self.fields:
            ...
            depth_column[mask] += -field.correction_results[
                "shift"
            ]  # TODO: adjust z-shift on angle
        ...
        out_df.to_csv(
            path, mode="a", index=False, header=False, sep="\t", encoding=encoding
        )
        return path

    # Visualize
    def show_lines(
        self, arrow_step=10, arrow_size=20, annotate_index=True, annotate_name=False
    ):
    ...
    def show_bubblemap(self, savepath=None):
        ...
        for key, intersection in self.intersections.items():
            ...
            size_ = 4 + (1 - corr) * 5
            color_ = (
                "red" if intersections_df.loc[[key]]["suspicious"].all() else "green"
            )
        ...
        fig.update_layout(
            title=f"2D SEG-Y<br>{len(self.intersections)} intersections",
            xaxis_title="CDP_X",
            yaxis_title="CDP_Y",
            width=1200,
            height=500,
...
@njit
def distribute_misties(
    a, b, n, skip_index=-1, max_iters=100, alpha=0.75, tolerance=0.00001
):
    ...
    for iteration in range(max_iters):
        ...
        if iteration != 0:
            stop_condition = (errors[iteration - 1] - errors[iteration]) / errors[
                iteration - 1
            ]
