...
class BaseGrid:
    ...
    def make_iterator(self):
        """Iterator that generates batches of locations."""
        return (
            self.locations[i : i + self.batch_size]
    ...
    def join(self, other):
        ...
        self_origin = (
            self.origin if isinstance(self, RegularGrid) else self.actual_origin
        )
        other_origin = (
            other.origin if isinstance(other, RegularGrid) else other.actual_origin
        )
        origin = np.minimum(self_origin, other_origin)

        self_endpoint = (
            self.endpoint if isinstance(self, RegularGrid) else self.actual_endpoint
        )
        other_endpoint = (
            other.endpoint if isinstance(other, RegularGrid) else other.actual_endpoint
        )
        endpoint = np.maximum(self_endpoint, other_endpoint)

        label_name = (
            other.label_name if isinstance(other, ExtensionGrid) else self.label_name
        )
    ...
    def show(self, grid=True, markers=False, n_patches=None, **kwargs):
        ...
        plotter = self.field.geometry.show(
            "dead_traces_matrix", cmap="Gray", colorbar=False, **kwargs
        )
        ax = plotter[0].ax
        ...
        for n in range(0, len(self), len(self) // n_patches - 1):
            slc = tuple(
                slice(o, e)
                for o, e in zip(self.locations[n, [3, 4]], self.locations[n, [6, 7]])
            )
            overlay[slc] = 1
...
class RegularGrid(BaseGrid):
    ...
    def __init__(
        ...
        overlap=None,
        overlap_factor=None,
        filtering_matrix="dead_traces_matrix",
        threshold=0,
        batch_size=64,
        field_id=-1,
        label_id=-1,
        ...
        **kwargs,
        ...
        # Make `strides`
        if (strides is not None) + (overlap is not None) + (
            overlap_factor is not None
        ) > 1:
            raise ValueError(
                "Only one of `strides`, `overlap` or `overlap_factor` should be specified!"
            )
        ...
        if strides is not None:
            strides = (
                strides
                if isinstance(strides, (tuple, list, np.ndarray))
                else [strides] * 3
            )
    ...
    def _make_locations(self):
        ...
        points = (
            np.array(np.meshgrid(i_grid, x_grid, d_grid, indexing="ij"))
            .transpose(order)
            .reshape(-1, 3)
        )
        ...
        if self.filtering_matrix is not None:
            points = filter_points(
                points, self.filtering_matrix, self.crop_shape, self.threshold
            )
    ...
    def __repr__(self):
        return (
            f"<RegularGrid for {self.field.short_name}: "
...
class RegularGridChunksIterator:
    ...
    def __init__(self, grid, size, overlap):
        ...
        if size_i is not None:
            step_i = (
                int(size_i * (1 - overlap_i))
                if np.issubdtype(type(overlap_i), np.floating)
                else size_i - overlap_i
            )
        else:
        ...
        if size_x is not None:
            step_x = (
                int(size_x * (1 - overlap_x))
                if np.issubdtype(type(overlap_x), np.floating)
                else size_x - overlap_x
            )
        else:
        ...
        if size_d is not None:
            step_d = (
                int(size_d * (1 - overlap_d))
                if np.issubdtype(type(overlap_d), np.floating)
                else size_d - overlap_d
            )
        else:
    ...
    def iterator(self):
        ...
        if self._iterator is None:
            ...
            grid_i = RegularGrid._arange(
                *grid.ranges[0], self.step_i, max(0, grid.endpoint[0] - self.size_i)
            )
            grid_x = RegularGrid._arange(
                *grid.ranges[1], self.step_x, max(0, grid.endpoint[1] - self.size_x)
            )
            grid_d = RegularGrid._arange(
                *grid.ranges[2], self.step_d, max(0, grid.endpoint[2] - self.size_d)
            )
...
class ExtensionGrid(BaseGrid):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.mode in ["best_for_all", "best_for_each", "best_for_each_independent"]:
            ...
            raise ValueError(
                "Provided wrong `mode` argument, for possible options look at the docstring."
            )
    ...
    def _make_locations(self):
        ...
        if "up" in self.directions:
            ...
            np.clip(
                buffer[directions_iterator, :, 2],
                0,
                self.field.shape[1],
                out=buffer[directions_iterator, :, 2],
            )
            np.clip(
                buffer[directions_iterator, :, 5],
                0,
                self.field.shape[1],
        ...
        if "down" in self.directions:
            ...
            np.clip(
                buffer[directions_iterator, :, 2],
                0,
                self.field.shape[1] - crop_shape[1],
                out=buffer[directions_iterator, :, 2],
            )
            np.clip(
                buffer[directions_iterator, :, 5],
        ...
        if "left" in self.directions:
            ...
            np.clip(
                buffer[directions_iterator, :, 1],
                0,
                self.field.shape[0],
                out=buffer[directions_iterator, :, 1],
            )
            np.clip(
                buffer[directions_iterator, :, 4],
                0,
                self.field.shape[0],
        ...
        if "right" in self.directions:
            ...
            np.clip(
                buffer[directions_iterator, :, 1],
                0,
                self.field.shape[0] - crop_shape[1],
                out=buffer[directions_iterator, :, 1],
            )
            np.clip(
                buffer[directions_iterator, :, 4],
        ...
        update_coverage_matrix = self.mode not in [
            "best_for_all",
        ...
        if self.mode in ["best_for_each", "best_for_each_independent"]:
            ...
            argsort = (
                potential.reshape(-1, n_directions)
                .argsort(axis=-1)[:, -self.top :]
                .reshape(-1)
            )

            # Shift argsorts to original indices
            shifts = np.repeat(
                np.arange(0, len(buffer), n_directions, dtype=np.int32), self.top
            )
            ...
            best_direction_idx = np.argmax(
                positive_potential.reshape(-1, n_directions).sum(axis=0)
            )
    ...
    def show(self, markers=False, overlay=True, frequency=1, **kwargs):
        ...
        self.field.geometry.show(
            "dead_traces_matrix", ax=ax, cmap="Grey", colorbar=False, **kwargs
        )
        ...
        if overlay:
            ...
            for n in range(0, len(self), frequency):
                slc = tuple(
                    slice(o, e)
                    for o, e in zip(
                        self.locations[n, [3, 4]], self.locations[n, [6, 7]]
                    )
                )
                overlay[slc] = 1
...
def compute_potential(
    ...
    for i, (orientation, i_start, x_start, _, i_stop, x_stop, _) in enumerate(
        locations
    ):
...
class LocationsPotentialContainer:
    ...
    def __init__(self, locations=None, potential=None):
        ...
        self.stats = {"repeated_locations": [], "total_repeated_locations": 0}
    ...
    def update_grid(self, grid):
        ...
        if len(new_locations) > 0:
            ...
            new_locations = new_locations.view(self.initial_dtype).reshape(
                -1, grid.locations.shape[1]
            )
        else:
