...
class VisualizationMixin:
    ...
    def default_plot_components(self):
        ...
        components = [
            items for items in components if all(hasattr(self, item) for item in items)
        ]
        return components
    ...
    def plot(
        ...
        augment_mask=True,
        augment_prediction=True,
        data_cmap="Greys_r",
        mask_cmap="gist_heat",
        mask_color="darkorange",
        **kwargs,
    ):
        ...
        config = {"scale": 0.8, **config, **kwargs}
    ...
    def plot_roll(
        ...
        **kwargs,
        ...
        for idx in indices:
            config_idx = self.get_plot_config(
                components=components,
                item_index=idx,
                zoom=zoom,
                add_suptitle=False,
        ...
        **kwargs,
        ...
        for idx in indices:
            ...
            frequencies = rfftfreq(
                data.shape[axis], sample_interval
            )  # `rfftfreq` is responsible for choosing units
            # of x-axis and expects `sample_spacing` in seconds

            for trace_idx in trace_indices:
                trace_idx_ = tuple(
                    np.insert(
                        np.array(trace_idx, dtype=np.object_),
                        insert_index,
                        slice(0, None),
                    )
                )
                ...
                plot_label.append(
                    f"FIELD: {displayed_name}   IDX: {idx}   TRACE: {trace_idx}"
                )

        plot_params = {
            "title": f"Spectrum of {src}-component",
            "label": plot_label,
            "xlabel": "Frequency, Hz",
            "ylabel": "Amplitude",
        }
        kwargs = {**plot_params, **kwargs}
