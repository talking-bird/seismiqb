...
class SeismicCropBatch(Batch, VisualizationMixin):
    ...
    def add_components(self, components, init=None):
        ...
        for component in to_list(components):
            if hasattr(self.dataset, component):
                msg = (
                    f"Component with `{component}` name cannot be added to batch, "
    ...
    def __setattr__(self, name, value):
        ...
        if (
            ...
            and value.ndim == 4
            and name not in self.name_to_order
        ):
    ...
    def preallocating_init(
        self, src=None, dst=None, buffer_type=None, return_indices=True, **kwargs
    ):
        ...
        if inplace:
            ...
            if src is None or buffer_type is not None:
                ...
                buffer = (getattr(np, buffer_type))(
                    (len(self), *self.crop_shape), dtype=dtype
                )
            else:
    ...
    def make_locations(self, generator, batch_size=None, keep_attributes=None):
        ...
        if callable(generator):
            ...
            raise ValueError(
                f"`generator` should either be callable or ndarray, got {type(generator)} instead!"
            )

        # Locations: 3D slices in the cube coordinates
        locations = [
            [slice(i_start, i_stop), slice(x_start, x_stop), slice(d_start, d_stop)]
        ...
        if keep_attributes:
            keep_attributes = (
                [keep_attributes]
                if isinstance(keep_attributes, str)
                else keep_attributes
            )
    ...
    @apply_parallel_decorator(
        init="preallocating_init", post="noop_post", buffer_type="empty", target="for"
    )
    def load_seismic(
        self, ix, buffer, dst, src=None, src_geometry="geometry", **kwargs
    ):
        ...
        field.load_seismic(
            locations=locations, src=src_geometry, buffer=buffer, **kwargs
        )

    load_cubes = load_crops = load_seismic

    @apply_parallel_decorator(
        init="preallocating_init", post="normalize_post", target="for"
    )
    def normalize(
        self,
        ix,
        ...
        buffer, stats = field.normalizer.normalize(
            buffer,
    ...
    def denormalize(self, ix, buffer, src, dst=None, mode=None, stats=None):
        ...
        buffer = field.normalizer.denormalize(
            buffer, normalization_stats=stats, mode=mode, inplace=True
        )
        return buffer
    ...
    @action
    @apply_parallel_decorator(
        init="preallocating_init", post="noop_post", buffer_type="zeros", target="for"
    )
    def create_masks(
        self,
        ix,
    ...
    @action
    @apply_parallel_decorator(init="indices", post="_assemble", target="for")
    def create_regression_masks(
        self, ix, dst, src=None, indices="all", src_labels="labels", scale=False
    ):
        """Create masks with relative depth."""
        field = self.get(ix, "fields")
        location = self.get(ix, "locations")
        return field.make_regression_mask(
            location=location, indices=indices, src=src_labels, scale=scale
        )

    @action
    @apply_parallel_decorator(init="indices", post="_assemble", target="for")
    def compute_label_attribute(
        self, ix, dst, src="amplitudes", atleast_3d=True, dtype=np.float32, **kwargs
    ):
        ...
        result = field.load_attribute(
            src=src, location=location, atleast_3d=atleast_3d, dtype=dtype, **kwargs
        )
        return result

    # Rebatch and its callables
    @action
    def rebatch_on_condition(
        self, src=None, condition="area", threshold=None, keep_attributes=None, **kwargs
    ):
        ...
        indices = np.array(
            [i for i, item in enumerate(data) if condition(item, **kwargs) > threshold]
        )
    ...
    def _compute_discontinuity_size(array, **kwargs):
        ...
        upper = np.max(
            np.array([mins, mins_next]), axis=0
        )  # maximum values of `min_depth` for each pixel

        maxs = points[:-1, 4][condition]
        maxs_next = points[1:, 4][condition]
        lower = np.min(
            np.array([maxs, maxs_next]), axis=0
        )  # minimum values of `max_depth` for each pixel
    ...
    def filter_sides(self, _, buffer, ratio, side, axis=0, src=None, dst=None):
        ...
        if not 0 <= ratio <= 1:
            raise ValueError(
                f"Invalid value ratio={ratio}: must be a float in [0, 1] interval."
            )
        ...
        locations[axis] = (
            slice(0, max_len - length) if side == "left" else slice(length, max_len)
        )
        buffer[tuple(locations)] = 0

    @apply_parallel_decorator(init="data", post="_assemble", target="for")
    def shift_masks(
        self, crop, n_segments=3, max_shift=4, min_len=5, max_len=10, src=None, dst=None
    ):
        ...
        for _ in range(n_segments):
            ...
            # Apply shift
            if shift != 0:
                segment_to_shift = crop[
                    :, begin : min(begin + length, crop.shape[1]), :
                ]
    ...
    @apply_parallel_decorator(init="data", post="_assemble", target="for")
    def linearize_masks(
        self, crop, n=3, shift=0, kind="random", width=None, src=None, dst=None
    ):
        ...
        if kind == "random":
            kind = np.random.choice(
                ["linear", "slinear", "quadratic", "cubic", "previous", "next"]
            )
        if width is None:
        ...
        *nz, y = np.nonzero(mask_)
        ...
        slc = (
            indices if axis == 0 else indices * 0,
    ...
    @action
    @apply_parallel_decorator(
        init="indices", post="_masks_to_horizons_post", target="for"
    )
    def masks_to_horizons(
        self, ix, src, dst, threshold=0.5, mode="mean", minsize=0, prefix="predict"
    ):
    ...
    def _masks_to_horizons_post(self, horizons_lists, dst=None, **kwargs):
        ...
        setattr(
            self,
            dst,
            [horizon for horizon_list in horizons_lists for horizon in horizon_list],
        )
        return self
    ...
    def save_masks(
        ...
        if horizons and len(horizons[-1]) > minsize:
            horizon = horizons[-1]
            str_location = "__".join(
                [f"{start}-{stop}" for start, stop in zip(origin, endpoint)]
            )
    ...
    def adaptive_expand(self, src, dst=None, axis=1, symbol="c"):
        ...
        for src_, dst_ in zip(src, dst):
            ...
            if data.ndim == 4 and data.shape[1] != 1:
                ...
                self.name_to_order[src_] = np.insert(
                    self.name_to_order[src_], axis - 1, symbol
                )
    ...
    def additive_noise(self, _, buffer, scale, **kwargs):
        ...
        """
        buffer += scale * self.random.standard_normal(
            dtype=np.float32, size=buffer.shape
        )
    ...
    def multiplicative_noise(self, _, buffer, scale, **kwargs):
        ...
        scale : float
            Standard deviation of normal distribution.
        """
        buffer *= 1 + scale * self.random.standard_normal(
            dtype=np.float32, size=buffer.shape
        )
    ...
    @apply_parallel_decorator(init="data", post="_assemble", target="for")
    def skeletonize_seismic(
        self, crop, smooth=True, axis=0, width=3, sigma=3, **kwargs
    ):
    ...
    def bandpass_filter(
        ...
        sos = butter(
            order, [lowcut, highcut], btype="band", output="sos", fs=sampling_frequency
        )
    ...
    @apply_parallel_decorator(
        init="preallocating_init", post="noop_post", target="for", requires_rng=True
    )
    def cutout_2d(
        self, _, buffer, patch_shape, n_patches, fill_value=0, rng=None, **kwargs
    ):
        ...
        upper_bounds = np.clip(
            np.array(buffer.shape) - np.array(patch_shape), a_min=1, a_max=buffer.shape
        )
    ...
    def fill_bounds(self, src, dst=None, margin=0.05, fill_value=0):
        ...
        for src_, dst_ in zip(src, dst):
            ...
            pad = [
                int(np.floor(s) * m) if isinstance(m, float) else m
                for m, s in zip(margin, crop.shape[1:])
            ]
