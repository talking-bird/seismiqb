    ...
    if simplices is not None:
        if colors is not None:
            fig = ff.create_trisurf(
                x=x, y=y, z=z, color_func=colors, simplices=simplices, **kwargs
            )
        else:
        ...
        for image, loc, axis in images:
            ...
            if resize_factor != 1:
                image = cv2.resize(
                    ...
                    tuple(np.array(shape)[::-1] // resize_factor),
                )
            image = image[::-1]
            if bounds:
                bounds = int(bounds)
            ...
            if axis == 0:
                x, y, z = (
                    loc * np.ones_like(image),
                    grid[0].T + zoom[1].start,
                    grid[1].T + zoom[2].start,
                )
            elif axis == 1:
                y, x, z = (
                    loc * np.ones_like(image),
                    grid[0].T + zoom[0].start,
                    grid[1].T + zoom[2].start,
                )
            else:
                z, x, y = (
