...
class HorizonExtractor:
    ...
    def init_container(self, array):
        ...
        for orientation in [0, 1]:
            for slide_idx in Notifier("t")(
                range(0, array.shape[orientation], self.step)
            ):
                ...
                lengths = {
                    item_idx: (
                        item_bbox[1 - orientation].stop
                        - item_bbox[1 - orientation].start
                    )
                ...
                self.container[orientation][slide_idx] = {
                    ...
                    "already_used": defaultdict(
                        bool
                    ),  # `item_idx` -> already merged or not
                }
    ...
    def get_points(self, orientation, slide_idx, item_idx):
        ...
        item_points = np.vstack(
            [item_indices[j] + (item_bbox[j].start + item_origin[j]) for j in range(3)]
        ).T
        return item_points
    ...
    def point_to_item_idx(self, orientation, point):
        ...
        item_idx = self.container[orientation][slide_idx]["labeled_slide"][
        ...
        for orientation, orientation_dict in self.container.items():
            for slide_idx, slide_dict in orientation_dict.items():
                for item_idx, item_length in slide_dict["lengths"].items():
                    if (
                        item_length > line_length_threshold
                        and slide_dict["already_used"][item_idx] is False
                    ):
                        iterator[(orientation, slide_idx, item_idx)] = item_length

        return sorted(iterator.items(), key=lambda item: item[1], reverse=True)

    def make_slide_line_sequence(
        self, orientation=0, slide_idx=None, line_length_threshold=50
    ):
        ...
        iterator = {}
        for item_idx, item_length in self.container[orientation][slide_idx][
            "lengths"
        ].items():
    ...
    def make_prototypes(
        self, sequence=None, n=None, line_length_threshold=50, max_iters=100, pbar="t"
    ):
        ...
        """
        sequence = sequence or self.make_line_sequence(
        ...
        with Notifier(pbar, total=n) as progress_bar:
            for (orientation, slide_idx, item_idx), _ in sequence:
                ...
                if (
                    self.container[orientation][slide_idx]["already_used"][item_idx]
                    is True
                ):
                    progress_bar.update()
                    continue
                ...
                prototype = self.init_prototype(
                    orientation=orientation, slide_idx=slide_idx, item_idx=item_idx
                )
                prototype = self.extend_prototype(
                    prototype=prototype,
    ...
    def init_prototype(self, orientation, slide_idx, item_idx):
        ...
        prototype.add_points(
            points=points, orientation=orientation, slide_idx=slide_idx
        )
        return prototype
    ...
    def extend_prototype(self, prototype, orientation, max_iters=100):
        ...
        for outer_iter in range(max_iters):
            ...
            for slide_idx in range(0, self.array.shape[orientation], self.step):
                ...
                intersections = prototype.get_intersections(
                    orientation=orientation, slide_idx=slide_idx
                )
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for prototype in prototypes:
                    future = executor.submit(
                        prototype.to_horizons,
                        field=field,
                        reduction=reduction,
                        d_ptp_threshold=d_ptp_threshold,
    ...
    def get_intersections(self, orientation, slide_idx):
        ...
        for points_dict in self.container[1 - orientation].values():
            for points in points_dict.values():
                if (
                    points[0, orientation] > slide_idx
                    or points[-1, orientation] < slide_idx
                ):
                    continue
                ...
                if (
                    insertion_index < len(points)
                    and points[:, orientation][insertion_index] == slide_idx
                ):
        ...
        for _ in range(n):
            ...
            for _ in Notifier(pbar)(range(max_iters)):
                ...
                for d_ptp_threshold_ in range(1, d_ptp_threshold, +3):
                    candidates = [
                        line_horizon
                        for line_horizon in line_horizons
                        if line_horizon.d_ptp <= d_ptp_threshold_
                    ]
                    horizon, _, _ = horizon.merge_into(
                        candidates, mean_threshold=0.01, max_threshold=0, adjacency=0
                    )
