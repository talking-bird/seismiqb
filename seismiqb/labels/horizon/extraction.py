...
class ExtractionMixin:
    ...
    def extract_from_mask(
        ...
        for _ in range(max_iters):
            ...
            for i, slices in enumerate(objects):
                ...
                if len(points) == len(grouped_points):
                    ...
                    mask[
                        horizon_points[:, 0], horizon_points[:, 1], horizon_points[:, 2]
                    ] = 0
                    num_deleted += 1
                    ...
                    for ix_coords, h_coords in surfaces:
                        ...
                        for inner_i, inner_slices in enumerate(inner_objects):
                            inner_indices = np.nonzero(
                                background[inner_slices] == inner_i + 1
                            )
                            inner_points = np.vstack(inner_indices).T

                            # Remove from the original mask
                            horizon_points = inner_points + [
                                (slc.start or 0) + (inner_slc.start or 0)
            ...
            if verbose:
                print(
                    f"Remaining points in the mask: {total_points:,}, num deleted: {num_deleted:>5}, "
        ...
        horizons = [
            cls(horizon_points + origin, field=field, name=f"{prefix}_{i}")
    ...
    def verify_merge(
        ...
        # Adjacency parsing
        adjacency = (
            adjacency if isinstance(adjacency, tuple) else (adjacency, adjacency)
        )
        adjacency_i, adjacency_x = adjacency

        # Overlap bbox
        overlap_i_min, overlap_i_max = (
            max(self.i_min, other.i_min),
            min(self.i_max, other.i_max) + 1,
        )
        overlap_x_min, overlap_x_max = (
            max(self.x_min, other.x_min),
            min(self.x_max, other.x_max) + 1,
        )
        ...
        if status != 1 and overlap_size_i > 0 and overlap_size_x > 0:
            self_overlap = self.matrix[
                overlap_i_min - self.i_min : overlap_i_max - self.i_min,
                overlap_x_min - self.x_min : overlap_x_max - self.x_min,
            ]
            other_overlap = other.matrix[
                overlap_i_min - other.i_min : overlap_i_max - other.i_min,
                overlap_x_min - other.x_min : overlap_x_max - other.x_min,
            ]

            mean_on_overlap, size_of_overlap = intersect_matrix(
                self_overlap, other_overlap, max_threshold
            )
            ...
            if size_of_overlap == 0:
                ...
                max_size_threshold is not None and size_of_overlap > max_size_threshold
            ):
                if mean_on_overlap < mean_threshold:
    ...
    def overlap_merge(self, other, inplace=False):
        ...
        shared_i_min, shared_i_max = (
            min(self.i_min, other.i_min),
            max(self.i_max, other.i_max),
        )
        shared_x_min, shared_x_max = (
            min(self.x_min, other.x_min),
            max(self.x_max, other.x_max),
        )

        background = np.zeros(
            (shared_i_max - shared_i_min + 1, shared_x_max - shared_x_min + 1),
            dtype=np.int32,
        )

        # Coordinates inside shared for `self` and `other`
        shared_self_i_min, shared_self_x_min = (
            self.i_min - shared_i_min,
            self.x_min - shared_x_min,
        )
        shared_other_i_min, shared_other_x_min = (
            other.i_min - shared_i_min,
            other.x_min - shared_x_min,
        )

        # Add both horizons to the background
        background[
            shared_self_i_min : shared_self_i_min + self.i_length,
            shared_self_x_min : shared_self_x_min + self.x_length,
        ] += self.matrix

        background[
            shared_other_i_min : shared_other_i_min + other.i_length,
            shared_other_x_min : shared_other_x_min + other.x_length,
        ] += other.matrix
    ...
    def adjacent_merge(self, other, mean_threshold=3.0, adjacency=3, inplace=False):
        ...
        overlap_d_min, overlap_d_max = (
            max(self.d_min, other.d_min),
            min(self.d_max, other.d_max),
        )
        ...
        dilated_background = dilate(background.astype(np.float32), kernel).astype(
            np.int32
        )
        ...
        if mergeable:
            background[(background < 0) & (background != self.FILL_VALUE)] -= (
                self.FILL_VALUE
            )
            background[background == 0] = self.FILL_VALUE
    ...
    def merge_into(
        ...
        if isinstance(horizons, (tuple, list)):
            horizons = np.array(
                [horizon for horizon in horizons if not horizon.already_merged]
            )

        # Pre-compute all the bounding boxes
        bboxes = np.array(
            [horizon.raveled_bbox for horizon in horizons], dtype=np.int32
        ).reshape(-1, 6)
        ...
        merge_stats.update(
            {"iteration_timings": [], "merge_candidates": [], "merges": []}
        )
        ...
        for _ in range(max_iters):
            ...
            for idx, merge_candidate in zip(indices, merge_candidates):
                ...
                merge_status = ExtractionMixin.verify_merge(
                    self,
                    merge_candidate,
                ...
                if merge_status == 4:
                    ...
                    merged = ExtractionMixin.overlap_merge(
                        self, merge_candidate, inplace=True
                    )

                elif merge_status == 3 and (adjacency_i > 0 or adjacency_x > 0):
                    # Adjacent horizons: maybe we can merge it
                    merged = ExtractionMixin.adjacent_merge(
                        self,
                        merge_candidate,
                        inplace=True,
                        mean_threshold=mean_threshold,
            ...
            merge_stats["iteration_timings"].append(
                round(perf_counter() - start_timing, 2)
            )
    ...
    def merge_list(
        ...
        for _ in range(max_iters):
            ...
            while True:
                ...
                for idx, merge_candidate in zip(indices, merge_candidates):
                    ...
                    if idx in indices_merged:
                        ...
                        continue
                    if (
                        id(merge_candidate) in current_horizon.id_separated
                        or id(current_horizon) in merge_candidate.id_separated
                    ):
                    ...
                    if merge_status == 4:
                        ...
                        merged = ExtractionMixin.overlap_merge(
                            current_horizon, merge_candidate, inplace=True
                        )
                        current_horizon.id_separated = (
                            current_horizon.id_separated.union(
                                merge_candidate.id_separated
                            )
                        )
                ...
                # Update bbox stats
                bboxes[i] = (
                    current_horizon.i_min,
                    current_horizon.i_max,
                    current_horizon.x_min,
                    current_horizon.x_max,
                    current_horizon.d_min,
                    current_horizon.d_max,
                )
            ...
            if adjacency_i <= 0 and adjacency_x <= 0:
                rejected_horizons_ = [
                    horizon for horizon in horizons if horizon.merge_count == 0
                ]
            ...
            merge_stats["global_iteration_timings"].append(
                round(perf_counter() - start_timing, 2)
            )
    ...
    @staticmethod
    def merge_list_concurrent(
        horizons,
        max_concurrent_iters=2,
        max_workers=16,
        min_workers=4,
        min_length=1000,
        multiplier=0.8,
        ...
        for _ in range(max_concurrent_iters):
            threading_flag, num_workers = ExtractionMixin._compute_threading_parameters(
                length=len(horizons),
            ...
            chunks = [
                horizons[idx : idx + chunk_size]
                for idx in range(0, len(horizons), chunk_size)
            ]
            ...
            with ThreadPoolExecutor(max_workers=len(chunks)) as executor:
                function = lambda horizons_list: ExtractionMixin.merge_list(
                    horizons_list,
                    adjacency=adjacency,
                    mean_threshold=mean_threshold,
                    max_threshold=max_threshold,
        ...
        horizons, counter = ExtractionMixin.merge_list(
            horizons,
            adjacency=adjacency,
            mean_threshold=mean_threshold,
            max_threshold=max_threshold,
    ...
    @staticmethod
    def _compute_threading_parameters(
        length, max_workers, min_workers, min_length, multiplier=0.8
    ):
        ...
        if num_workers >= min_workers:
            return ExtractionMixin._compute_threading_parameters(
                length, num_workers, min_workers=min_workers, min_length=min_length
            )
