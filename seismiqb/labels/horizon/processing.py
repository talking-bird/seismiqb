...
class ProcessingMixin:
    ...
    # Filtering methods
    def filter(
        self, filtering_matrix=None, margin=0, inplace=False, add_prefix=True, **kwargs
    ):
    ...
    def filter_disconnected_regions(self, erosion_rate=0, inplace=False):
        ...
        if erosion_rate > 0:
            filtering_matrix = binary_dilation(
                filtering_matrix, structure, iterations=erosion_rate
            )
    ...
    def smooth_out(
        ...
        kernel = (
            ...
            else make_gaussian_kernel(kernel_size=kernel_size, sigma=sigma_spatial)
        )
        ...
        result[
            self.field.dead_traces_matrix[
                self.i_min : self.i_max + 1, self.x_min : self.x_max + 1
            ]
        ] = self.FILL_VALUE
    ...
    def interpolate(
        ...
        kernel = (
            kernel
            if kernel is not None
            else make_gaussian_kernel(kernel_size=kernel_size, sigma=sigma)
        )
        ...
        for _ in range(iters):
            result = _interpolate(
                src=result,
                kernel=kernel,
                min_present_neighbors=min_present_neighbors,
    ...
    def inpaint(
        ...
        :class:`~.Horizon`
            Processed horizon instance. A new instance if `inplace` is False, `self` otherwise.
        """
        image = self.matrix.astype(
            np.uint16
        )  # dtype conversion for compatibility with the OpenCV method
        dead_traces_matrix = self.field.dead_traces_matrix[
            self.i_min : self.i_max + 1, self.x_min : self.x_max + 1
        ]
        ...
        result = cv2_inpaint(
            src=image,
            inpaintMask=holes_mask,
            inpaintRadius=neighbors_radius,
            flags=method,
        )
        result = result.astype(self.dtype)

        # Filtering mask to remove traces, that are too far from existing traces
        too_far_traces_mask = binary_erosion(
            holes_mask, iterations=inpaint_radius
        ).astype(int)
        ...
        result[
            (too_far_traces_mask == 1)
            | (anomalies_mask == 1)
            | (dead_traces_matrix == 1)
        ] = self.FILL_VALUE
    ...
    def generate_holes_matrix(
        ...
        if isinstance(scale, float):
            ...
            while len(scales) < n:
                new_scales = rng.exponential(
                    scale, size=sampling_scale * (n - len(scales))
                )
        ...
        for location, figure_scale in zip(locations, scales):
            ...
            figure_coordinates = make_bezier_figure(
                n=n_key_points,
                radius=radius,
                sharpness=sharpness,
                scale=figure_scale,
            ...
            negative_coords_shift = np.min(
                np.vstack([figure_coordinates, [0, 0]]), axis=0
            )
            huge_coords_shift = np.max(
                np.vstack([figure_coordinates - self.shape, [0, 0]]), axis=0
            )
        ...
        if noise_level:
            noise = rng.normal(
                loc=coordinates, scale=noise_level, size=coordinates.shape
            )
    ...
    def make_holes(
        ...
        filtering_matrix = self.generate_holes_matrix(
            n=n,
            scale=scale,
            max_scale=max_scale,
    ...
    make_holes.__doc__ += "\n" + "\n".join(
        generate_holes_matrix.__doc__.split("\n")[1:]
    )
