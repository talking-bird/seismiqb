...
class AttributesMixin:
    ...
    def matrix_put_on_full(self, matrix):
        ...
        if matrix.shape[:2] != self.field.spatial_shape:
            background = np.full(
                shape=self.field.spatial_shape,
                fill_value=self._dtype_to_fill_value(matrix.dtype),
                dtype=matrix.dtype,
            )
            background[self.i_min : self.i_max + 1, self.x_min : self.x_max + 1] = (
                matrix
            )
        else:
    ...
    def _matrix_absence_mask(self, matrix):
        ...
        msg = (
            f"Can't define horizon absence mask with respect to provided matrix since its shape {matrix.shape} "
    ...
    def matrix_dilate(self, matrix, dilation_iterations=1):
        ...
        fill_value = (
            np.nan if issubclass(matrix.dtype.type, np.floating) else self.FILL_VALUE
        )
        ...
        matrix = dilate(
            matrix, kernel=np.ones((3, 3), np.uint8), iterations=dilation_iterations
        )
    ...
    def pca_transform(data, n_components=3, **kwargs):
        ...
        result = np.full((*data.shape[:2], n_components), np.nan).reshape(
            -1, n_components
        )
    ...
    def filled_matrix(self):
        ...
        return self.filled_full_matrix[
            self.bbox[0, 0] : self.bbox[0, 1] + 1, self.bbox[1, 0] : self.bbox[1, 1] + 1
        ]
    ...
    def probabilities(self):
        ...
        if hasattr(self, "proba_points"):
            ...
            _map[
                self.proba_points[:, 0].astype(np.int32),
                self.proba_points[:, 1].astype(np.int32),
            ] = self.proba_points[:, 2]
        ...
        raise AttributeError(
            f"Horizon `{self.displayed_name}` hasn't `proba_points` attribute. Check, whether"
    ...
    def get_cube_values(
        ...
        **_,
        ...
        geometry = (
            ...
            else getattr(self.field, src_geometry)
        )
        ...
        result = np.full(
            (*self.field.spatial_shape, window), fill_value=np.nan, dtype=np.float32
        )
    ...
    def get_layer_values(self, other, window=0, offset=0, src_geometry=None):
        ...
        n = d_stop - d_start
        result = np.full(
            (*presence_matrix.shape, n), fill_value=np.nan, dtype=np.float32
        )
    ...
    ALIAS_TO_ATTRIBUTE = {
        alias: name for name, aliases in ATTRIBUTE_TO_ALIAS.items() for alias in aliases
    }
    ...
    def load_attribute(
        self, src, location=None, use_cache=True, enlarge=False, **kwargs
    ):
        ...
        if src_name in self.ATTRIBUTE_TO_METHOD:
            ...
            data = self.get_property(
                src_name, use_cache=use_cache, enlarge=enlarge, **kwargs
            )
    ...
    def get_property(self, src, **_):
        ...
        if data is None:
            ...
            raise ValueError(
                f"Unknown `src` {src}. Expected a matrix-property or one of {aliases}."
            )
        return data
    ...
    def get_instantaneous_amplitudes(self, window=11, offset=0, **kwargs):
        ...
        """
        amplitudes = self.get_cube_values(
            window=window, offset=offset, use_cache=False, **kwargs
        )
    ...
    def get_wavelet_decomposition(self, widths=range(1, 14, 3), window=50, **_):
        ...
        for idx, width in enumerate(widths):
            ...
            result[:, :, idx] = convolve(amplitudes, wavelet, mode="constant")[
                :, :, window // 2
            ]
    ...
    @lru_cache(maxsize=1, apply_by_default=False, copy_on_return=True)
    @transformable
    def get_spectral_decomposition(
        self,
    ...
    def get_zerocrossings(self, side, window=15):
        ...
        """
        values = self.get_cube_values(
            window=window, offset=window // 2 * side, fill_value=0
        )
    ...
    @lru_cache(maxsize=1, apply_by_default=False, copy_on_return=True)
    @transformable
    def get_median_diff_map(
        self,
        iters=2,
        window_size=11,
        max_depth_difference=0,
        threshold=2,
    ...
    def get_gradient_map(self, threshold=1, dilation_iterations=2, **_):
        ...
        grad_i = self.load_attribute(
            "grad_i", on_full=True, dtype=np.float32, use_cache=False
        )
        grad_x = self.load_attribute(
            "grad_x", on_full=True, dtype=np.float32, use_cache=False
        )
    ...
    def get_spikes_mask(
        ...
        max_spike_width=7,
        min_spike_size=5,
        max_depths_distance=2,
        dilation_iterations=0,
        nan_amount_to_ignore=1,
        points_distance=3,
    ):
        ...
        for rotation_num in range(1, 5):
            ...
            rotated_spikes = _get_spikes_along_line(
                matrix=matrix,
    ...
    for line_idx in range(matrix.shape[0]):
        ...
        while previous_idx < line_length - points_distance:
            ...
            if isnan(previous_depth) and (not isnan(current_depth)):
                if (
                    current_idx - previous_not_nan_idx
                    < nan_amount_to_ignore + points_distance
                ):
            ...
            for spike_potential_point_idx in range(
                spike_start_idx + 1, spike_potential_end_idx + 1
            ):
                ...
                if not isnan(depth):
                    ...
                    if depths_diff <= max_depths_distance:
                        spikes_mask[
                            line_idx, spike_start_idx:spike_potential_point_idx
                        ] = 1
...
def _medfilt(src, window_size, preserve_missings, max_depth_difference):
    ...
    for iline in prange(0, i_range):
        for xline in range(0, x_range):
            ...
            element = src[
                max(0, iline - k) : min(iline + k + 1, i_range),
                max(0, xline - k) : min(xline + k + 1, x_range),
            ].ravel()
