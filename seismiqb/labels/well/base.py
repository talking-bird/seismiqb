...
class Well:
    ...
    def match_to_seismic(self, columns=("X", "Y", "TWT"), field=None, well=None):
        ...
        assert field is not None, (
            "Making points requires field reference in either `Well` instance or method call!"
        )

        #
        cdp_xy_values = self.data[list(columns[:2])].values
        ordinal_xy_values = field.lines_to_ordinals(
            field.cdp_to_lines(cdp_xy_values).astype(np.int32)
        )
        ...
        #
        matched_well = well or Well(
            storage=field, location=ordinal_xy_values[0], name=f"matched_{self.name}"
        )
        matched_well.merge_data(other=data, key_self="TIME", key_other=columns[-1])
        matched_well.data = matched_well.data.astype(
            {"INLINE_3D": np.int32, "CROSSLINE_3D": np.int32}
        )
        ...
        points = matched_well.data.reset_index()[
            ["INLINE_3D", "CROSSLINE_3D", "SAMPLES"]
        ]
        ...
        for column in matched_well.keys:
            ...
            matched_well.bboxes[column] = np.array(
                [[i_min, i_max], [x_min, x_max], [d_min, d_max]], dtype=np.int32
            )
        return matched_well
    ...
    def compute_filtered_log(
        self, log, order=5, frequency=60, btype="lowpass", name=None
    ):
        """Apply filtration to a given log."""
        array = self.data[log].values
        filtered_array = self._compute_filtered_log(
            array,
            order=order,
            frequency=frequency,
            btype=btype,
            fs=self.field.sample_rate,
        )
    ...
    @staticmethod
    def _compute_filtered_log(
        array, despike=None, order=5, frequency=60, btype="lowpass", fs=500
    ):
        if despike is not None:
            array = scipy.signal.medfilt(array, despike)

        sosfilt = scipy.signal.butter(
            order, frequency, btype=btype, fs=fs, output="sos"
        )
    ...
    def compute_overlap_mask(self, mask_bbox, log="AI"):
        ...
        mask = (self.points[:, -1] >= mask_bbox[-1][0]) & (
            self.points[:, -1] < mask_bbox[-1][1]
        )
        if self.vertical is False:
            mask &= (self.points[:, 0] >= mask_bbox[0][0]) & (
                self.points[:, 0] < mask_bbox[0][1]
            )
            mask &= (self.points[:, 1] >= mask_bbox[1][0]) & (
                self.points[:, 1] < mask_bbox[1][1]
            )
    ...
    def add_to_mask(self, mask, locations, log="AI", **kwargs):
        ...
        if overlap_mask is not None:
            ...
            mask[points[:, 0], points[:, 1], points[:, 2]] = self.data[log].values[
                overlap_mask
            ]
        return mask
    ...
    def __repr__(self):
        return (
            f"""<Well `{self.name}` for `{self.field.short_name}` at {hex(id(self))}>"""
        )
        ...
        **kwargs,
        ...
        if horizontal_layout:
            ...
            kwargs = {
                ...
                "xlabel_size": 20,
                "xlim": (
                    [index_lim] * n_subplots if combine == "separate" else index_lim
                ),
                "ylabel": "",
                "window": 100,
                "alpha": 0.7,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for well in additional_wells:
            main_well.merge_data(
                well, key_self="index", key_other="index", inplace=True
            )
