...
class FaultVisualizationMixin(VisualizationMixin):
    ...
    def compute_auto_zoom(self, index, axis, zoom_margin=20):
        ...
        return tuple(
            slice(
                max(self.bbox[i][0] - zoom_margin, 0),
    ...
    def show(self, axis=0, zoom="auto", **kwargs):
        ...
        return self.show_slide(
            index=int(np.mean(self.bbox[axis])), zoom=zoom, axis=axis, **kwargs
        )
    ...
    def show_3d(
        ...
        **kwargs,
        ...
        x, y, z, simplices = self.make_triangulation(
            zoom, sticks_step, stick_nodes_step, sticks=sticks
        )
    ...
    def make_triangulation(
        ...
        **kwargs,
        ...
        if (
            (sticks_step is not None and sticks_step != self.sticks_step)
            or (
                stick_nodes_step is not None
                and stick_nodes_step != self.stick_nodes_step
            )
            or (
                stick_orientation is not None
                and stick_orientation != self.stick_orientation
            )
        ):
            # sticks must be recreated with new parameters from points

            temporary_fault = type(self)(
                {"points": self.points}, field=self.field, direction=self.direction
            )
            temporary_fault.points_to_sticks(
                slices,
                sticks_step or 10,
                stick_nodes_step or 10,
        ...
        if sticks:
            ...
            faults = [
                type(self)(
                    ...
                    stick_orientation=self.stick_orientation,
                    field=self.field,
                    name=(self.short_name or "") + "_" + str(i),
                )
            ...
            return [
                np.concatenate(data) if len(data) > 0 else data
                for data in [x, y, z, simplices]
            ]
        ...
        if len(self.sticks) == 1 and len(self.sticks[0]) > 1:
            ...
            return fake_fault.make_triangulation(
                slices, sticks_step, stick_nodes_step, **kwargs
            )
...
def get_fake_one_stick_fault(fault):
    ...
    fake_fault = type(fault)(
        {"sticks": np.array([stick, stick + 1])},
        stick_orientation=fault.stick_orientation,
