...
class Fault(FaultSticksMixin, FaultSerializationMixin, FaultVisualizationMixin):
    ...
    def set_direction(self, direction):
        ...
        if direction is None:
            if self.has_component("sticks") and len(self.sticks) > 0:
                ...
                self.direction = (
                    0 if depth_slice[:, 0].ptp() > depth_slice[:, 1].ptp() else 1
                )
    ...
    def load(cls, path, field, name=None, interpolate=False, **kwargs):
        """Load faults."""
        if not isinstance(path, str) or os.path.splitext(path)[1][1:] not in [
            "char",
            "",
        ]:
            ...
            faults = [
                cls(df, field=field, name=name, **kwargs)
                for name, df in cls.split_charisma(path).items()
            ]
        ...
        if self._simplices is None:
            if self._points is None and self._sticks is None:
                raise AttributeError(
                    "'simplices' can't be created ('points' and 'sticks' don't exist)"
                )
    ...
    def nodes(self):
        ...
        if self._nodes is None:
            if self._points is None and self._sticks is None:
                raise AttributeError(
                    "'nodes' can't be created ('points' and 'sticks' don't exist)"
                )
    ...
    def points(self):
        ...
        if self._points is None:
            if self._simplices is None and self._sticks is None:
                raise AttributeError(
                    "'points' can't be created ('nodes'/'simplices' and 'sticks' don't exist)"
                )
            if len(self.simplices) > 1:
                ...
                self._points = points[
                    points[:, self.direction] == self.sticks[0][0, self.direction]
                ]
    ...
    def sticks(self):
        ...
        if self._sticks is None:
            if self._simplices is None and self._points is None:
                raise AttributeError(
                    "'sticks' can't be created ('nodes'/'simplices' and 'points' don't exist)"
                )
    ...
    def simplices_to_points(self, width=1):
        ...
        for triangle in self.simplices:
            points.append(
                triangle_rasterization(self.nodes[triangle].astype("float32"), width)
            )
        self._points = np.concatenate(points, axis=0).astype("int32")

    def points_to_sticks(
        self,
        slices=None,
        sticks_step=10,
        stick_nodes_step=10,
        ...
        self._sticks = points_to_sticks(
            points=points,
            sticks_step=sticks_step,
            nodes_step=stick_nodes_step,
            fault_orientation=self.direction,
    ...
    def add_to_mask(
        self, mask, locations=None, width=1, axis=None, sparse=False, alpha=1, **kwargs
    ):
        ...
        mask_bbox = np.array(
            [
                [locations[0].start, locations[0].stop],
        ...
        if (self.bbox[:, 1] < mask_bbox[:, 0]).any() or (
            self.bbox[:, 0] >= mask_bbox[:, 1]
        ).any():
            return mask
        ...
        if sparse and self.has_component("sticks"):
            ...
            loc = loc[
                np.logical_and(
                    mask_bbox[self.direction, 0] <= loc,
                    loc < mask_bbox[self.direction, 1],
                )
            ]

            points = points[np.isin(points[:, self.direction], loc)]

            unlabeled_slides = take_along_axis(
                mask, loc - mask_bbox[self.direction, 0], self.direction
            )
        ...
        insert_points_into_mask(
            mask, points, mask_bbox, width=width, axis=1 - self.direction, alpha=alpha
        )
        return mask
