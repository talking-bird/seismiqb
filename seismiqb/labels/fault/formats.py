...
class FaultSticksMixin(CharismaMixin):
    ...
    def read_df(cls, path):
        ...
        with open(path, encoding="utf-8") as file:
            line_len = len(
                [item for item in file.readline().split(" ") if len(item) > 0]
            )
        ...
        if line_len == 3:
            ...
            raise ValueError(
                "Fault labels must be in FAULT_STICKS, CHARISMA or REDUCED_CHARISMA format."
            )
    ...
    def split_df_into_sticks(self, df, return_direction=False):
        ...
        xlines_diff = sum(
            df["CROSSLINE_3D"][1:].values - df["CROSSLINE_3D"][:-1].values == 0
        )
        ...
        if col is None:
            raise ValueError(
                "Wrong format of sticks: there is no column to group points into sticks."
            )

        df = df.sort_values("DEPTH")
        sticks = (
            df.groupby(col)
            .apply(lambda x: x[self.COLUMNS].values)
        ...
        **kwargs,
        ...
        if len(sticks) > 1:
            ...
            coords = pca.fit_transform(
                np.array([stick[0][:2] for stick in sticks.values])
            )
        ...
        if direction is not None:
            ...
            if (ptp > 2).any():
                warnings.warn(
                    f"{self.name}: there sticks on several slides in both directions"
                )
        ...
        for filename in glob.glob(path):
            ...
            try:
                ...
                if any(len(item) == 1 for item in sticks):
                    ...
                    continue
                mask = sticks.apply(
                    lambda x: len(np.unique(np.array(x)[:, 2])) == len(x)
                )
                if not mask.all():
...
class FaultSerializationMixin:
        ...
        self.from_dict(
            {
                ...
                "sticks": (
                    self._labeled_array_to_sticks(sticks, sticks_labels)
    ...
    def load_sqb(self, path):
        ...
        self.from_dict(
            {key: storage[key] for key in ["points", "nodes", "simplices", "sticks"]}
        )
