...
parser.add_argument(
    "-f", "--force", action="store_true", help="recreate cube if exists"
)
parser.add_argument(
    "-s", "--save", action="store_true", help="whether to save output notebook"
)
parser.add_argument(
    "-p", "--postfix", action="store_true", help="add postfix to the created file"
)
parser.add_argument("--format", default="qsgy", help="format of cube")
parser.add_argument(
    "--nonconvert", action="store_true", help="whether cubes should be converted"
)
parser.add_argument(
    "--nonquantize", action="store_true", help="disable quantization of cube"
)
...
with open(os.devnull, "w") as fnull:
    with redirect_stderr(fnull) as err:
        exceptions = run_notebook(
            PATH, INPUTS, inputs_pos=3, display_links=False, out_path_ipynb=OUTPUT_PATH
        )
